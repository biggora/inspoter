...
Виджет погоды — единственное место, откуда приложение обращается к внешнему сервису:
серверный запрос к `api.open-meteo.com` с координатами из настроек виджета, ответ
кешируется в памяти процесса на 10 минут. Если такой запрос нежелателен, не добавляйте
этот виджет — остальные работают только на данных самого приложения.
