...
| AC-MAIL-008      | `tests/unit/services/mail-accounts.test.ts` : "testConnection — returns imapOk/smtpOk true for a MOCK config" · `tests/unit/mail/smtp-transport.test.ts` : SSL/STARTTLS option mapping, split SMTP timeouts, host:port + transport code in the failure detail, `mail:smtp` log source · `tests/unit/ui/mail-account-dialog.test.tsx` : blocked-port hint shown for ETIMEDOUT and withheld for EAUTH · `e2e/mail-client.spec.ts` fixture creates the MOCK account through the settings flow | PASS   |
...
| ID         | Acceptance/evidence target                                                                                                                                                                                                                                                                                                                              | Trace          | Status |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ------ |
| DSH-UI-001 | All eleven widget kinds render from a payload fixture and from their empty payload without crashing; each empty state prints its own explanation rather than blank space.                                                                                                                                                                               | AC-DSH-007/015 | PASS   |
...
| DSH-UI-006 | Messages tile names its channel and category, falls back for a message with no author and no text, and prints its own empty copy. Its settings form narrows the channel list to the chosen category, drops ticks that no longer belong to it, keeps them when the category is cleared, adds rather than replaces a tick, and toggles the unread filter. | AC-DSH-008/015 | PASS   |
...
| DSH-SVC-008 | Messages widget resolution: only the ticked channels, newest first, each row named by channel and category; an untouched tick list yields the whole category; ticks outside the chosen category are ignored; the unread filter and the item limit apply; a deleted channel yields an empty tile rather than every channel; no cross-workspace leakage. | AC-DSH-008/015     | PENDING |
