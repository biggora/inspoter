...
| --- | ------ | ---- | ------------------------ |
| 1.1 | **PRD v3.1.** Внести принятые решения Phase 0: заменить NFR-I18N-001 (Q-1); добавить безусловный FR-MSG-003 и точные AC-MSG-009..014 для сообщений оператора (Q-4); резолвить OQ-1..4/6/7/9; добавить §3.11 «Real provider enablement» c AC-REAL-* и критерием готовности продукта, сформулированным от реальной ценности. Каждое требование получает трассировку к idea.md или Q-ID; требования без источника удаляются. | product-analyst | prd.md v3.1; doc-review PASS; ни одного требования без источника |
...
| 2.2 | **Webhook backbone, Logs, tokens + workspace facet.** Atomic idempotency, direct/compound ownership, all 401/400/201/429/413 paths, rate limit, one-time secret/revoke, pagination, header/stale-context/API isolation. | backend-dev + database engineer + frontend-dev → tester → code-reviewer | AC-WH/LOG plus Logs/tokens facets of AC-WS-008/010/011 PASS; review PASS |
| 2.3 | **Domains/DNS mock facet.** Workspace-exclusive manifest mocks/bindings, record CRUD/validation, provider error isolation, foreign-binding zero-call, cache/cursor/header/stale-tab behavior. | backend-dev + frontend-dev → tester → code-reviewer | AC-DOM/PROV Domains facet + workspace facet PASS; review PASS |
| 2.4 | **Servers mock facet.** Workspace-exclusive servers, deterministic power state, confirmation/error/reconcile, foreign-binding zero-call, header/stale-tab behavior. | backend-dev + frontend-dev → tester → code-reviewer | AC-SRV/PROV Servers facet + workspace facet PASS; review PASS |
| 2.5 | **Alerts facet.** Durable direct ownership, strict optional category pair/`SET NULL`, delete, ingest, filtering/pagination, header/cache/cursor isolation. | database engineer + backend-dev + frontend-dev → tester → code-reviewer | AC-ALR + Alerts facet of AC-WS-008/010/011 PASS; review PASS |
| 2.6 | **Mail facet.** Direct ownership, webhook ingest, filters/keyset cursor bound to workspace, header/stale-tab isolation. | database engineer + backend-dev + frontend-dev → tester → code-reviewer | AC-MAIL + Mail facet of AC-WS-008/010/011 PASS; review PASS |
| 2.7 | **Messages facet.** Compound category/channel/message ownership, operator/webhook origin, missing-channel 4xx, pagination cursor/header/stale-tab isolation. | database engineer + backend-dev + frontend-dev → tester → code-reviewer | AC-MSG-009..014 plus active Messages and workspace facets PASS; review PASS |
...
| 4.3 | **Messages compose — довести до спеки (Q-4, безусловно).** Валидация, автор-оператор vs webhook-источник, error-состояния; точные AC-MSG-009..014 покрыты тестами в 2.7. | backend-dev + frontend-dev | AC-MSG-009..014 PASS |
| 4.4 | **Demo-seed (Q-12, безусловная задача).** Реализовать опциональную команду `npm run db:seed:demo`: представительные данные для всех разделов; идемпотентно и отдельно от production workspace. | backend-dev | Свежая установка + demo-seed → ни одного пустого раздела; повторный запуск не дублирует |
