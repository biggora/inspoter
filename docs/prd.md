...
- Description: A workspace owner creates rules for one Mail account and one target label. Rules evaluate eligible incoming webhook and IMAP messages using exact sender and subject-substring predicates; populated predicates combine with AND. A sender predicate is at most 320 characters, a subject predicate at most 200 characters, and each account has at most 100 active rules. Applied assignments persist independently of later rule edits, disablement, or deletion. Optional existing-message application is off by default and runs as a durable resumable backfill. This requirement is sourced only from the verbatim Mail-label request and the reviewed `specs/mail-label-filtering-plan.md` v0.3.
- Priority: Must Have (Mail labels slices)
- Acceptance Criteria:
  - **AC-MAIL-039**: Given an authenticated workspace owner, one active-workspace Mail account, and one active-workspace target label, When the owner creates a rule with an 80-character-or-shorter name and at least one supported predicate (exact sender of at most 320 characters or subject substring of at most 200 characters), Then one active rule is persisted for that account and label; Given 100 active rules already exist for the account, When concurrent create/enable operations attempt to exceed the limit, Then transactional account-scoped locking permits no more than 100 and each excess operation returns 409 `ACTIVE_RULE_LIMIT_REACHED`; Given a member, When they attempt to create or mutate a rule or label definition, Then the request is rejected with 403 and no write occurs.
  ...
  - **AC-MAIL-043**: Given an opened Mail message, When an owner invokes **Filter messages like this**, Then the rule dialog is prefilled with that message's account and sender, permits edits plus target-label selection or creation before submit, and closing the dialog restores focus to its trigger.
  - **AC-MAIL-044**: Given a rule that has already applied labels, When an owner edits, disables, or deletes that rule, Then existing message-label assignments remain unchanged while only subsequent eligible message evaluation uses the new rule state.
  - **AC-MAIL-045**: Given an owner explicitly selects the initially unchecked **Apply to existing mail** option, When historical application runs, Then it selects only that rule's account messages in folders whose `specialUse` is `INBOX`, traverses the immutable eligible set in ascending `(createdAt, id)` order after an exclusive cursor and through an inclusive captured cutoff, and processes at most 200 rows per atomic batch; `processed` counts eligible rows inspected and `matched` counts predicate matches even when the assignment already exists. Given a batch failure, lease expiry, live-ingestion overlap, or manual retry, Then assignments/counts/cursor never advance without the same commit, pre-labeled matches remain idempotent, post-cutoff messages are handled only by live evaluation, a five-minute lease can be renewed or taken over after expiry, three consecutive failed worker attempts transition the run to `FAILED`, and manual retry resets consecutive attempts to zero while preserving snapshot, cutoff, committed cursor/counts, and assignments.
...
Members may use normal content, DNS, server power actions, create workspaces, list members, manually assign/remove existing Mail labels, and filter the Mail list by label. Owners have those permissions and additionally manage workspace metadata, membership, provider discovery/claim/removal, binding transfer, Mail label definitions, and automatic Mail filter rules. A member's label-definition or rule mutation returns 403 without a write (AC-MAIL-031/039); member manual assignment/removal and label filtering remain allowed (AC-MAIL-032/036). This is the complete `OWNER`/`MEMBER` distinction for this iteration; granular RBAC remains out of scope. AC-WS-008, AC-WS-010, and AC-WS-011 remain `PARTIAL` by facet through R2.7 and become `PASS`, and the workspace family 11/11, only after the R2.8 two-workspace/two-member all-section runtime gate.
...
| N-27 | A member attempts to create/edit/delete a label definition or automatic rule.                                                                                             | HTTP 403; no definition, rule, or assignment changes.                                                                                                                                                                                                                               | AC-MAIL-031, AC-MAIL-039                                      |
...
No v3.12 requirement is gated by an unresolved product question. Q-1…Q-13, Q-14, and Q-15 are confirmed user decisions.
...
- **Q-15 — RESOLVED, BINDING (2026-07-20):** implement the approved `specs/mail-label-filtering-plan.md` v0.3: local workspace labels, owner-managed label definitions/rules, member manual assignment/filtering, one-account incoming rules with exact-sender and subject-substring AND semantics, one target label, persistent assignments across rule lifecycle, and optional initially-unchecked durable historical application. Provider label sync, regex/OR, nesting, body matching, non-label actions, and private labels remain deferred. The user explicitly approved phased implementation; no open question gates Phase 1.
