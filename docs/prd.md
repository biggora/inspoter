...
4. View, filter, sort, and label mail, including automatic account-scoped label rules; view, filter, and sort logs and alerts.
...
- Description: An operator creates any number of named dashboards in the active workspace at `/dashboards`. Boards are workspace-shared — every member sees and edits the same set, like bookmarks or services. One board may be pinned as the section's start board and `/dashboards` forwards to it. Without a pinned board the section falls back to the first board by creation order. The application post-login home is `/management`.
...
- Acceptance Criteria:
  ...
  - **AC-DSH-005**: Given an authenticated operator with at least one board, When they open `/dashboards`, Then they land on the start board (or the first board by position when none is pinned).
...
- Description: Widgets occupy cells on a 12-column grid. Moving and resizing are available only after the operator enters edit mode; in view mode tiles stay interactive (links, scrolling) and cannot be displaced by a stray press. A tile displaced by another is pushed down, freed space is pulled back up, and the layout is validated against per-kind size limits and non-overlap on the server as well as in the browser. Every kind may grow vertically to the shared 20-row limit regardless of neighbouring tile heights; width limits remain kind-specific.
- Priority: Must Have (Dashboards slice)
- Acceptance Criteria:
  ...
  - **AC-DSH-012**: Given edit mode, When the operator resizes a tile by pointer or by focusing its resize control and pressing an arrow key, Then the tile changes size by whole cells within its limits — including vertical growth to the shared 20-row maximum independent of neighbouring tile heights — and the new layout persists and survives a reload.
...
### 3.0b Executive Management

**FR-MGT-001: Workspace executive home**

- Description: `/management` is the first, non-hideable authenticated route for
  one company/workspace. `OWNER` and `MEMBER` have the same management rights;
  MEMBER is presented as Manager without changing the stored role.
- Acceptance Criteria:
  - **AC-MGT-001**: Authenticated root and default post-login navigation resolve
    to `/management`, while an explicitly validated local `next` still wins.
  - **AC-MGT-002**: Management cannot be hidden and `/dashboards` remains an
    independent functional section.
  - **AC-MGT-003**: A member of one workspace cannot read or mutate management
    data in another workspace.

**FR-MGT-002: Deterministic executive snapshot and briefs**

- Description: A bounded, redacted local snapshot summarizes operational work,
  Calendar, communications and active decisions. An ordinary editable Agent may
  publish a daily or weekly Brief from the exact hashed snapshot through private
  agent-only tools. The deterministic snapshot and manual Decisions work without
  an LLM credential.
- Acceptance Criteria:
  - **AC-MGT-004**: Canonical snapshot JSON is stable, at most 128 KiB and omits
    credentials, bodies, attachments, HTML and raw provider payloads.
  - **AC-MGT-005**: Public MCP, WebMCP, `/api/v1`, webhook-token scopes and the
    public OpenAPI document expose no management mutation or publication tool.
  - **AC-MGT-006**: Concurrent/retried generation publishes at most one Brief
    per Generation and accepts evidence only from that Generation's snapshot.

**FR-MGT-003: Human-controlled Decisions**

- Description: Humans may create, edit, defer, approve and reject Decisions.
  AI may only propose Decisions while publishing a Brief. Approval can create
  one typed artifact: Kanban card, standard one-shot Reminder, Note or local Mail
  draft. Sending and arbitrary agent continuation are prohibited.
- Acceptance Criteria:
  - **AC-MGT-007**: Every mutation uses optimistic `version`; action payload
    changes use a separate `actionRevision`.
  - **AC-MGT-008**: Target, immutable Receipt, authoritative Event and required
    durable work commit atomically; concurrent approve/retry creates one target.
  - **AC-MGT-009**: A committed Receipt makes the action permanently
    non-repeatable, including after backup/restore.
  - **AC-MGT-010**: Import executes no action; interrupted work requires an
    explicit retry or rebind and successful history remains terminal.
...
- Acceptance Criteria (under FR-MAIL-003/004):
  ...
  - **AC-MAIL-030**: Given several accounts (including the system webhook mailbox), When the operator marks one as the workspace default in `/settings/mail`, Then Mail preselects that account on an ordinary first load, exactly one account carries the flag, and deleting it promotes the oldest remaining IMAP account (or the webhook mailbox when no IMAP account remains). An explicit valid deep link still overrides the default. When the operator switches the selected account, Then the folder list, message list, and reading pane all switch to that account's content; foreign-workspace account/folder/message ids return a non-disclosing not-found response (consistent with AC-WS-011).

**FR-MAIL-008: Workspace Mail labels**

- Description: Inspoter stores labels locally within one workspace and persists many-to-many assignments to Mail messages. Any authenticated member of the active workspace may manage label definitions, manually assign/remove labels, and filter the Mail list. Label text remains the primary identity, while color is supplementary. A workspace has at most 100 labels. Label names are normalized in this exact order for uniqueness: Unicode NFKC, trim surrounding whitespace, collapse each internal whitespace run to one ASCII space, then locale-stable lowercase. This requirement is sourced from the verbatim Mail-label request, the reviewed `specs/mail-label-filtering-plan.md` v0.3, and the v3.14 authorization amendment.
- Priority: Must Have (Mail labels slices)
- Acceptance Criteria:
  - **AC-MAIL-031**: Given an authenticated member of the active workspace, When they create, rename, recolor, reorder, or delete an unreferenced label with a name of at most 40 characters, Then the change is persisted only in the active workspace; Given two names that become equal after NFKC → trim → collapse internal whitespace runs to one ASCII space → locale-stable lowercase, When the second is submitted, Then it is rejected with 409 `LABEL_NAME_CONFLICT`; Given 100 labels already exist, When concurrent creates attempt to exceed the limit, Then transactional workspace-scoped locking permits no more than 100 and each excess create returns 409 `LABEL_LIMIT_REACHED`; Given a label referenced by any active or inactive rule, When deletion is attempted, Then it returns 409 `LABEL_IN_USE`, while deleting an unreferenced label cascades its message assignments.
  - **AC-MAIL-032**: Given a Mail message and label in the active workspace and an authenticated owner or member, When the same add operation or the same remove operation is repeated, Then each operation succeeds idempotently and at most one message-label assignment exists.
  - **AC-MAIL-033**: Given a message with applied labels, When it is shown in the Mail list or opened in the reading pane, Then the list renders label chips beside the subject and the reading pane renders the complete applied-label set.
  - **AC-MAIL-034**: Given labeled messages at 1440px and 375px viewport widths, When a row contains more labels than fit the documented limit, Then the list shows at most two chips on desktop or one chip on the narrow layout plus a `+N` overflow indicator, while row activation remains available and the page body has no horizontal overflow.
  - **AC-MAIL-035**: Given any displayed label or label selection control, When it is viewed visually or through assistive technology, Then the label name is visibly or accessibly exposed and label identity never depends on color alone.
  - **AC-MAIL-036**: Given account, folder, label, unread, text-query, and sort facets, When the operator combines or changes them, Then `labelId` intersects the other active facets, the keyset cursor is bound to the normalized workspace/filter fingerprint, and pagination resets safely instead of mixing results from a different workspace or filter state.
  - **AC-MAIL-037**: Given a label, rule, account, or message identifier owned by another workspace, When it is supplied to any label, assignment, filter-rule, or Mail-list operation, Then the server returns a non-disclosing 404, performs no cross-workspace write, and does not reveal whether the foreign resource exists.
  - **AC-MAIL-038**: Given labeled messages in a Mail list response, When the list endpoint returns one bounded page, Then each item may include only bounded label metadata (`id`, `name`, `color`) and includes neither message bodies nor attachment bytes.

**FR-MAIL-009: Automatic Mail label rules**

- Description: Any authenticated member of the active workspace may create and manage rules for one Mail account and one target label. Rules evaluate eligible incoming webhook and IMAP messages using exact sender and subject-substring predicates; populated predicates combine with AND. A sender predicate is at most 320 characters, a subject predicate at most 200 characters, and each account has at most 100 active rules. Applied assignments persist independently of later rule edits, disablement, or deletion. Optional existing-message application is off by default and runs as a durable resumable backfill. This requirement is sourced from the verbatim Mail-label request, the reviewed `specs/mail-label-filtering-plan.md` v0.3, and the v3.14 authorization amendment.
- Priority: Must Have (Mail labels slices)
- Acceptance Criteria:
  - **AC-MAIL-039**: Given an authenticated member of the active workspace, one active-workspace Mail account, and one active-workspace target label, When they create a rule with an 80-character-or-shorter name and at least one supported predicate (exact sender of at most 320 characters or subject substring of at most 200 characters), Then one active rule is persisted for that account and label; Given 100 active rules already exist for the account, When concurrent create/enable operations attempt to exceed the limit, Then transactional account-scoped locking permits no more than 100 and each excess operation returns 409 `ACTIVE_RULE_LIMIT_REACHED`; Given an operator without membership in the active workspace, When they attempt to create or mutate a rule or label definition, Then the request is rejected and no write occurs.
  - **AC-MAIL-040**: Given a rule and candidate incoming message, When each populated predicate is normalized with Unicode NFKC, surrounding-whitespace trimming, and locale-stable lowercase conversion, Then sender uses exact comparison, subject uses substring comparison, and both predicates must match when both are populated.
  - **AC-MAIL-041**: Given the same active matching rule, When a future message is persisted by webhook into the system account's `INBOX` folder or imported/re-imported by IMAP into a folder whose `specialUse` is `INBOX`, Then the target label assignment is committed atomically with the message; messages in Archive, Junk/Spam, Trash, Drafts, or Sent are ineligible, and moving an existing message into INBOX does not re-evaluate it. A UIDVALIDITY-driven INBOX re-import is an eligible import, so automatic rules reapply.
  - **AC-MAIL-042**: Given retries, concurrent ingestion, overlapping rules, multiple rules targeting the same label, or a concurrent rule create/edit/disable/delete, When an eligible message transaction acquires the account-scoped transaction advisory lock and then loads the active-rule snapshot in `(position, id)` order, Then mutations committed before that snapshot affect the message and mutations committed after it do not; the unique message-label pair is created at most once, and message persistence, attachment metadata, and all resulting assignments commit atomically.
  - **AC-MAIL-043**: Given an opened Mail message, When an active-workspace member invokes **Filter messages like this**, Then the rule dialog is prefilled with that message's account and sender, permits edits plus target-label selection or creation before submit, and closing the dialog restores focus to its trigger.
  - **AC-MAIL-044**: Given a rule that has already applied labels, When an active-workspace member edits, disables, or deletes that rule, Then existing message-label assignments remain unchanged while only subsequent eligible message evaluation uses the new rule state.
  - **AC-MAIL-045**: Given an active-workspace member explicitly selects the initially unchecked **Apply to existing mail** option, When historical application runs, Then it selects only that rule's account messages in folders whose `specialUse` is `INBOX`, traverses the immutable eligible set in ascending `(createdAt, id)` order after an exclusive cursor and through an inclusive captured cutoff, and processes at most 200 rows per atomic batch; `processed` counts eligible rows inspected and `matched` counts predicate matches even when the assignment already exists. Given a batch failure, lease expiry, live-ingestion overlap, or manual retry, Then assignments/counts/cursor never advance without the same commit, pre-labeled matches remain idempotent, post-cutoff messages are handled only by live evaluation, a five-minute lease can be renewed or taken over after expiry, three consecutive failed worker attempts transition the run to `FAILED`, and manual retry resets consecutive attempts to zero while preserving snapshot, cutoff, committed cursor/counts, and assignments.
  - **AC-MAIL-046**: Given an active-workspace member opens a message and the workspace has an LLM credential, When they request a summary, Then a read-only summary of that message is returned and displayed with a notice that it was generated by a model, nothing is written to the message, and selecting another message discards it.
  - **AC-MAIL-047**: Given an active-workspace member has the reply composer open for a message, When they request an AI draft, Then the returned text replaces the composer body and is marked unsaved; nothing is persisted until the existing draft autosave runs, and nothing is sent without an explicit send action.
  - **AC-MAIL-048**: Given an active-workspace member requests a filter-rule suggestion for a message, When the model answers, Then the ordinary rule form opens pre-filled with the suggested name, match mode and conditions and with no label selected; no rule, label, folder assignment or identifier is created by the model, and the rule exists only after the member submits the form.
  - **AC-MAIL-049**: Given a suggested condition whose field/operator pair, value length or `HAS_ATTACHMENT` value the deterministic filter engine cannot accept, When the suggestion is prepared, Then that condition is discarded, the remaining valid conditions are offered up to the 10-condition maximum, and the number discarded is shown to the member; Given every suggested condition is invalid, Then the request fails with an invalid-response error and no form opens.
  - **AC-MAIL-050**: Given the workspace has no LLM credential, When a member requests a summary, a draft or a suggestion, Then the request is rejected with HTTP 501 `AI_UNAVAILABLE`, the member is told that no model is configured and pointed at provider settings, nothing is journaled, and no request leaves the machine.
  - **AC-MAIL-051**: Given the workspace exceeds `LLM_CALL_RATE_LIMIT` model calls within `LLM_CALL_RATE_WINDOW_MS`, When another AI request is made, Then it is rejected with HTTP 429 `AI_RATE_LIMIT` and a message naming the window, and the refused call reaches no model and is not journaled.
  - **AC-MAIL-052**: Given the model returns text that is not a JSON object of the expected shape, When the answer is validated, Then the request fails with HTTP 502 `AI_INVALID_RESPONSE`, and neither the error shown to the member nor the internal log entry contains any part of the model output.
  - **AC-MAIL-053**: Given a message body longer than the prompt budget, When any AI request is made for it, Then only the leading portion is sent to the model, quoted history and HTML are excluded, and the member is told the message was truncated.
  - **AC-MAIL-054**: Given several LLM credentials in a workspace, When a member marks one as default at `/settings/providers`, Then that credential answers subsequent AI requests, the flag is cleared from every other credential of the same category, and no other category is affected; Given no credential is marked, Then the oldest LLM credential answers.

**Automatic-rule eligibility and concurrency contract:** Live evaluation and historical application use the identical eligible set: the rule's account plus a folder whose persisted `specialUse` is `INBOX`. Webhook intake qualifies because it persists into the system account's INBOX. IMAP initial/incremental imports and UIDVALIDITY re-imports qualify only in an INBOX-special-use folder. Archive, Junk/Spam, Trash, Drafts, Sent, and folders without INBOX special-use are excluded. Existing-message move operations never trigger evaluation. Every rule create/edit/enable/disable/delete and eligible message-persistence transaction uses the same account-scoped transaction advisory lock; active rules load only after lock acquisition. The snapshot boundary is the database commit order: a rule mutation committed before snapshot load is visible to that message, while one committed later is not.

**Historical-run contract:** Run creation stores an immutable predicate/target-label snapshot and an inclusive maximum `(cutoffCreatedAt, cutoffId)` from eligible INBOX rows for the account. The first batch has no lower bound; later batches select tuples strictly greater than the committed cursor and less than or equal to the cutoff, ordered ascending, limit 200. Each batch atomically writes conflict-safe assignments, increments `processed` for every selected row and `matched` for every predicate match regardless of a pre-existing assignment, and advances the cursor to the final inspected tuple. A crash before commit repeats the batch; a crash after commit starts strictly after its cursor. Claim sets a five-minute lease; a worker renews before expiry, and another worker may claim only after expiry. `attempts` counts consecutive failed worker attempts: each terminal attempt failure increments it, the third failure sets `FAILED` and clears the lease; manual retry is allowed only from `FAILED`, resets attempts to zero, clears the lease/error, sets `PENDING`, and preserves immutable snapshot, cutoff, committed cursor/counts, and assignments. Messages created after the cutoff are excluded from the run and evaluated by the live path; conflict-safe assignment makes live/backfill overlap idempotent.

**Transactional limits and conflicts:** Label create holds a workspace-scoped transaction advisory lock while counting the 100-label maximum. Rule create and enable hold the account-scoped rule/message lock while counting the 100-active-rule maximum. Boundary and concurrent requests return deterministic 409 conflicts (`LABEL_LIMIT_REACHED` or `ACTIVE_RULE_LIMIT_REACHED`) rather than overshooting. Label delete cascades message assignments only when no active or inactive rule references it; any such rule reference returns 409 `LABEL_IN_USE`.
...
8. **UIDVALIDITY reset and local labels (FR-MAIL-008/009):** automatic labels reapply during a folder re-import; manually applied labels may be lost because the current reset deletes and recreates messages, and nullable/non-unique `Message-ID` values are not used to guess identity.
...
**FR-WH-003: Discord-compatible webhooks (both directions)**

- Description: Inspoter speaks Discord's webhook wire format so integrations already able to post to a Discord channel work against Inspoter by changing one URL, and so Inspoter events can be delivered to a Discord channel or to a receiver written against Discord Webhook Events. The normative contract, including every accepted-and-ignored field and every deliberate deviation, is [`specs/discord-webhook-compatibility.md`](../specs/discord-webhook-compatibility.md). The existing contracts of FR-WH-001 and FR-WH-002 are unchanged and remain supported.
- Priority: Should Have
- Acceptance Criteria:
  - **AC-WH-012** (Ingress): Given an existing channel webhook credential and a Discord Execute Webhook payload sent to `POST /api/discord/webhooks/<webhook-id>/<token>`, When it is received, Then a channel message is created with the payload's `content`, `username`, `avatar_url` and up to ten `embeds`, and the response is **HTTP 204** with no body, or **HTTP 200** with a Discord message object when `wait=true`. No new credential is issued and the native channel route keeps working with the same pair.
  - **AC-WH-013** (Discord error shape): Given a Discord request that carries nothing displayable, exceeds a documented limit, is unparseable, is oversized, presents an unknown or revoked credential, or exceeds the rate limit, When it is received, Then the response uses Discord's error shape — `{ message, code, errors? }` with `50006`, `50035`, `40005` or `401: Unauthorized`, or the `{ message, retry_after, global }` body plus `X-RateLimit-*` headers for **HTTP 429** — and no message is created.
  - **AC-WH-014** (Egress to Discord): Given an outgoing webhook whose format is `DISCORD_EXECUTE`, When a subscribed event fires, Then the delivery body is a Discord Execute Webhook payload carrying one embed derived from the event, and a `429` response is retried after the interval its `retry_after` states rather than the generic backoff ladder.
  - **AC-WH-015** (Egress as Discord): Given an outgoing webhook whose format is `DISCORD_EVENTS`, When it is created and when a subscribed event fires, Then the receiver is sent a `type: 0` PING and then `type: 1` event envelopes, each signed with Ed25519 over `timestamp + body` in `X-Signature-Ed25519` / `X-Signature-Timestamp` and verifiable with the public key shown to the operator, and repeated terminal failures disable the webhook automatically.
...
Members may use normal content, DNS, server power actions, create workspaces, list members, and manage Mail accounts, labels, assignments, and automatic filter rules within the active workspace. Owners have those permissions and additionally manage workspace metadata, membership, provider discovery/claim/removal, and binding transfer. Operators without active-workspace membership cannot access or mutate its Mail configuration. This is the complete `OWNER`/`MEMBER` distinction for this iteration; granular RBAC remains out of scope. AC-WS-008, AC-WS-010, and AC-WS-011 remain `PARTIAL` by facet through R2.7 and become `PASS`, and the workspace family 11/11, only after the R2.8 two-workspace/two-member all-section runtime gate.
...
### 3.13 Workspace backup & restore

**FR-BCK-001: Encrypted workspace export by section**

- Description: A workspace owner exports selected sections of the active workspace into a single encrypted `.inspot-backup` file from `/settings/backup` (`POST /api/backup/export`). Any non-empty subset of the nine sections is selectable: bookmarks, messages, mail, logs, alerts, services, webhooks, providers, workspaceSettings. The archive is sealed with an operator-supplied passphrase (minimum 10 characters): AES-256-GCM over gzip(JSON), key derived with scrypt (format v1, `src/lib/backup/format.ts`). The payload carries a manifest (schemaVersion 1, exportedAt, appVersion, workspace metadata incl. hiddenSections, selected sections, per-model counts) plus per-model record arrays. Operators, memberships, sessions, external identities, webhook delivery attempts, and idempotency keys are never exported.
- Priority: Must Have (backup slice)
- Acceptance Criteria:
  - **AC-BCK-008**: Given an export with only a subset of sections selected (e.g. bookmarks only), When the archive is produced and later imported, Then the payload contains only the selected sections' model arrays; a replace import deletes only sections present in the archive (a pre-existing log entry survives a bookmarks-only replace); and a webhooks-only archive (without the messages section) imports channel-scoped tokens with `channelId` nulled out.

**FR-BCK-002: Import with replace/merge and id regeneration**

- Description: A workspace owner restores an archive into the active workspace (`POST /api/backup/import`, multipart `{file, passphrase, mode}`) in one of two modes. `merge` (default) adds the archive's records to existing content; `replace` first deletes the workspace rows of exactly the sections present in the archive (children first) and then imports. In both modes every id is regenerated with foreign-key remapping, and the whole import runs in a single interactive transaction (timeout `BACKUP_IMPORT_TX_TIMEOUT_MS`). Transient state is reset on import: mail account `syncStatus` → `IDLE` (next sync scheduled immediately), service `nextCheckAt` → now, provider binding `operationState` → `IDLE` with a cleared operation lease. Collision guards are global to the instance: a webhook token whose `tokenHash` already exists and a provider binding whose natural key `(provider, accountKey, resourceType, mode, remoteId)` already exists are skipped and counted; importing while the source workspace still exists on the same instance therefore skips those rows — the disaster-recovery path (source gone) imports them fully. An archive is memory-bound in v1 and limited by `BACKUP_MAX_IMPORT_BYTES` (413 over the limit).
- Priority: Must Have (backup slice)
- Acceptance Criteria:
  - **AC-BCK-001**: Given a workspace with content in every section exported with all nine sections, When the archive is merge-imported into a fresh workspace, Then a full copy of every exported model is created with newly generated ids and remapped relations (category parent/child hierarchy preserved), original `createdAt` timestamps, BigInt fields (e.g. mail folder `uidValidity`) and attachment bytes preserved exactly, an imported provider credential decrypts to the original secret under the local `CREDENTIAL_ENCRYPTION_KEY`, and the imported mail account's `syncStatus` is reset to `IDLE`.
  - **AC-BCK-002**: Given a workspace with pre-existing content, When an archive is imported in replace mode, Then the rows of every section present in the archive are wiped and replaced so per-model counts equal the archive's manifest counts exactly (no accumulation), pre-existing extra rows in those sections are removed, and the archive manifest's `hiddenSections` are applied to the workspace.
  - **AC-BCK-003**: Given the same workspace an archive was exported from, When the archive is merge-imported back into it, Then per-model counts double with newly generated ids (bookmarks and categories are never reused by name) and no unique-constraint violation occurs, except that an `AlertCategory` with the same name is reused instead of duplicated and colliding rows are skipped per AC-BCK-005.
  - **AC-BCK-004**: Given an import with a wrong passphrase, a file truncated below the 53-byte header, or a version byte other than 1, When the import is attempted, Then it is rejected with `BACKUP_PASSPHRASE_INVALID_OR_CORRUPT`, `BACKUP_INVALID_FILE`, or `BACKUP_UNSUPPORTED_VERSION` respectively (HTTP 400) and nothing is imported; a wrong passphrase and a tampered/corrupt ciphertext are intentionally indistinguishable (single AES-GCM authentication failure).
  - **AC-BCK-005**: Given an archive containing a webhook token whose `tokenHash` already exists on the instance (globally unique), When the archive is merge-imported, Then that token row is skipped and counted in the summary's `skipped` counter while the original token row continues to exist uniquely and keeps working; the same global skip rule applies to a `ProviderResourceBinding` whose natural key already exists.
  - **AC-BCK-006**: Given a merge import of an archive containing the system WEBHOOK mail account into a workspace that already has its own WEBHOOK account, When the import completes, Then no second WEBHOOK-kind account is created — the archive's account is remapped onto the existing one and its folders are reused by `path` — the archive's mail items land in the reused folder, and the source workspace's own webhook account and items are untouched.
  - **AC-BCK-007**: Given a workspace `MEMBER`, When they invoke export or import, Then the request is rejected (403, `requireWorkspaceOwner` at the service boundary) and nothing is exported or imported; Given the workspace `OWNER`, Then the same call is authorized.

**FR-BCK-003: Secret portability via the archive passphrase**

- Description: Encrypted-at-rest secrets (`MailAccount` passwords, `OutgoingWebhook` signing secrets, `ProviderCredential` payloads) are exported **decrypted** as `secretData` inside the sealed archive — the archive passphrase is the sole protection in transit — and re-encrypted with the target deployment's local `CREDENTIAL_ENCRYPTION_KEY` at import. This makes an archive portable across deployments with different encryption keys. A workspace secret that cannot be decrypted at export fails with `BACKUP_SECRET_DECRYPT_FAILED` (500); a deployment without `CREDENTIAL_ENCRYPTION_KEY` rejects export/import with 503.
- Priority: Must Have (backup slice)
- Acceptance Criteria: no additional IDs — the executable assertions are the secret round-trip inside AC-BCK-001 (imported credential decrypts to the original value under the local key) and the sealed-container guarantees of AC-BCK-004.

---

### 3.14 Agent surface (2026-08-23)

FR-MSG-004 and FR-CNT-006 opened Messages and Contacts to an API token. This section extends the same contract to the remaining operator-facing sections and states the limits that hold across all of them. Nothing here changes what an operator can do in the interface, and no requirement above is amended.

**FR-AGT-001: Agent surface across the operator-facing sections**

- Description: Every section an operator works in daily is reachable by an API token rather than a browser session, published twice over one service layer — as MCP tools on `POST /api/mcp` and as the REST family `/api/v1/**` — under the read/write scope pair of the section on the existing workspace token (FR-WH-002). Bookmarks (`bookmarks:*`), Kanban (`kanban:*`), Mail (`mail:*`) and monitored Services (`services:*`) join Messages (FR-MSG-004) and Contacts (FR-CNT-006); Alerts, Servers and Logs remain read-only. Both surfaces answer the same question the same way because the shared predicates live in the service layer, not in either adapter.
- Priority: Should Have
- Acceptance Criteria:
  - **AC-AGT-001** (Scoped token access): Given a request to either surface, When it presents no token, an unknown or revoked token, a channel-scoped webhook token, or a token carrying no MCP scope, Then it is rejected with `401` and a bearer challenge; When the token's scopes lack the operation's requirement, Then it is rejected with `403` — and in both cases nothing is read or written.
  - **AC-AGT-002** (Workspace isolation): Given an identifier belonging to another workspace, When it is used against either surface, Then the response is a non-disclosing `404` (REST) or a tool error (MCP), and no record is read or modified in either workspace.
  - **AC-AGT-003** (Surface parity): Given an operation published on both surfaces, When it is invoked through MCP and through `/api/v1`, Then both reach the same service function and produce the same change, including the outgoing webhooks a kanban card write raises.
  - **AC-AGT-004** (Attribution): Given a write through the REST family, Then it is journalled in `Activity` under the token's id and name; Given a row that records its own author — a kanban comment or a message — Then it carries the token's id and name, and a message additionally carries `MessageOrigin.AGENT`.

**FR-AGT-002: What a token may not do**

- Description: Three classes of action are withheld from both surfaces. First, a delete that cascades into content the caller never saw: a kanban board or column, a bookmark category, a Messages category or channel, and a mail account — extending AC-MSG-018's rule to every section. Deleting a leaf (card, checklist item, comment, bookmark, label, message, service, contact) is allowed. Second, mail account credentials: creating, editing, deleting and testing an IMAP/SMTP account would carry a password through an assistant's context, so accounts stay an operator action in `/settings/mail` while reading, organizing, sending, filter rules and on-demand sync are published. Third, server power actions: `servers:read` has no write counterpart.
- Priority: Must Have
- Acceptance Criteria:
  - **AC-AGT-005** (No cascading delete): Given a token holding every scope, When it lists the available tools or the documented REST paths, Then no operation deletes a kanban board or column, a bookmark category, a Messages category or channel, or a mail account; and the guarantee is pinned by name so adding such an operation fails the suite first.
  - **AC-AGT-006** (No mail credentials): Given a token holding `mail:read` and `mail:write`, When it lists mail accounts, Then no password or password hint is returned, and no operation creates, edits, deletes or tests an account.
  - **AC-AGT-007** (Author-only comment deletion): Given a kanban comment written by an operator or by a different token, When a token attempts to delete it, Then the response is the same non-disclosing `404` a foreign id would produce and the comment remains.

**FR-AGT-003: One documented contract**

- Description: `specs/openapi.json` documents every `/api/v1` path and is rendered at `/settings/api-docs`. The inventory of paths and the methods allowed on each is pinned in exactly one place, `scripts/check-public-openapi.mjs`, which the full CI profile runs before the rest of the suite; the unit assertions and the api-docs end-to-end test read the specification rather than restating it. Shared auth, limit and error answers are declared once under `components.responses`.
- Priority: Should Have
- Acceptance Criteria:
  - **AC-AGT-008** (Documented surface): Given the specification, When the contract gate runs, Then the declared paths and methods match the pinned inventory exactly, every operation carries a unique operation id and bearer security, and the rendered reference at `/settings/api-docs` shows those paths and no others.

**Known documentation gap:** Kanban and monitored Services have no feature requirements of their own in this document — they were built after v3.19 and were never written up. FR-AGT-001..003 describe only their agent surface. Writing the two feature sections is outstanding work, not a decision.
...
**NFR-A11Y-001 (Accessibility baseline):** Interactive controls (navigation, forms, list actions) must be keyboard operable and have accessible names; status and label identity must not depend on color alone; dialogs must restore focus. The UI should meet WCAG 2.1 AA for color contrast and focus visibility on the shell and Bookmarks section at minimum. _Verification:_ automated accessibility checks (e.g., axe) pass with no critical violations on Slice 1 screens and no serious/critical violations on the Mail label flows; AC-MAIL-034/035/043 pass at desktop and 375px widths.

**NFR-I18N-001 (English default, Russian and Latvian fully supported):** The finite allowlist of operator-visible non-translated terms, unchanged regardless of the active English, Russian, or Latvian locale, is: `inspoter`, `Cloudflare`, `Hetzner Cloud`, `Hetzner DNS`, `GoDaddy`, `DNS`, `VPS`, `IP`, `URL`, `TTL`, `HTTP`, `HTTPS`, `API`, and `JSON`. Technical identifiers, environment-variable names, and API snippets are exempt only when rendered as code or monospace. All unlisted operator-visible navigation, headings, labels, actions, validation messages, errors, loading states, empty states, and notifications must be fully localized in the active locale (English by default, Russian or Latvian when selected via the top-bar language switcher). Legacy English visible copy in `specs/ui.md` is non-normative placeholder text and must be replaced with proper localized strings for all three supported languages. _Verification:_ an automated visible-string scan against this allowlist plus a browser walkthrough of every route, dialog, loading state, error, and empty state finds no unlocalized text in any supported locale (Q-1, superseded — see docs/design.md Changelog).
...
**NFR-DESIGN-001 (Normative design conformance):** Visible layout, flows, interaction, and component behavior must conform to `specs/prototype/`, `specs/inspot-design/`, and `specs/ui.md`; the reviewed `specs/mail-label-filtering-plan.md` v0.3 supplies the additive Mail-label interaction contract. Q-3 makes those design properties normative, not legacy English strings, which remain subject to NFR-I18N-001. Q-13 confirms the all-content workspace switcher behavior; Q-4 supersedes only the read-only Messages statement. _Verification:_ the design audit checks every section, Mail label chips/rule dialog/sidebar/filter interaction, stale-workspace recovery, keyed remount, loading/error/empty states, and no-full-page-reload switch flow with no unexplained material delta.
...
| HC-5  | All visible UI defaults to English and is fully supported in Russian and Latvian via an operator-selectable language switcher, except the finite allowlist in NFR-I18N-001 (which applies to all three languages) and code/monospace technical exemptions.                                                                                                                         | Q-1                                                                                             |
...
| HC-16 | Mail labels are local, workspace-scoped Inspoter data; owners manage label definitions and automatic rules, while owners and members may manually assign labels and filter Mail. Rules target one account and incoming messages only, apply one label, support exact sender and subject substring with AND semantics, and never remove persisted assignments on lifecycle changes. | Q-15; verbatim Mail-label request; `specs/mail-label-filtering-plan.md` v0.3                    |
...
| A-8 | Confirmed Mail-label scope: optional historical application is unchecked by default and delivered only as a durable, bounded, resumable backfill; provider-side labels and private labels are not inferred.                                                                                                                                          | Q-15; verbatim Mail-label request; reviewed plan v0.3                                                                                                                                                                                  | Mail labels and rules     | high       | User      | Verify FR-MAIL-008..009 and AC-MAIL-031..045                  |
...
- **Mail source** — mail enters through webhook ingest (FR-MAIL-002) into the system webhook mailbox, and, since Q-14, through IMAP synchronization of operator-connected accounts (FR-MAIL-004). Both incoming persistence paths apply local active rules (FR-MAIL-009); outbound mail uses the account's SMTP transport (FR-MAIL-006) and is not eligible for incoming rules.
...
| N-26 | A label name duplicates after normalization, a concurrent create exceeds 100 labels, or delete targets a rule-referenced label.                                           | Return 409 `LABEL_NAME_CONFLICT`, `LABEL_LIMIT_REACHED`, or `LABEL_IN_USE` respectively; no duplicate/101st label or partial delete occurs, and an unreferenced delete alone cascades assignments.                                                                                  | AC-MAIL-031                                                   |
| N-27 | An operator without active-workspace membership attempts to create/edit/delete a label definition or automatic rule.                                                      | Request is rejected; no definition, rule, or assignment changes.                                                                                                                                                                                                                    | AC-MAIL-031, AC-MAIL-039                                      |
| N-28 | A label/rule/account/message id belongs to another workspace.                                                                                                             | Non-disclosing HTTP 404 before any write; no existence disclosure or cross-workspace mutation.                                                                                                                                                                                      | AC-MAIL-037                                                   |
| N-29 | Concurrent/retried ingestion, overlapping rules, or a concurrent rule mutation targets one eligible message.                                                              | Under the account lock, the post-lock active-rule snapshot includes mutations committed before it and excludes later commits; exactly one assignment exists and the message/assignments commit atomically.                                                                          | AC-MAIL-041..042                                              |
| N-30 | A filter rule is edited, disabled, or deleted after matching historical mail.                                                                                             | Existing assignments remain; only future evaluation changes.                                                                                                                                                                                                                        | AC-MAIL-044                                                   |
| N-31 | Historical application crashes before/after commit, overlaps live labeling, loses its five-minute lease, fails three consecutive worker attempts, or is manually retried. | Exclusive-cursor/inclusive-cutoff traversal resumes from the last atomic commit; post-cutoff mail stays live-only; the third consecutive failure sets `FAILED`; manual retry resets attempts while preserving committed snapshot/progress; no duplicate assignment or count occurs. | AC-MAIL-045                                                   |
| N-32 | Concurrent create/enable attempts exceed 100 active rules for one account.                                                                                                | Account-scoped transactional locking permits at most 100 active rules; excess operations return 409 `ACTIVE_RULE_LIMIT_REACHED`.                                                                                                                                                    | AC-MAIL-039                                                   |
| N-33 | A message is imported outside INBOX or an existing message is moved into INBOX.                                                                                           | No automatic rule evaluation occurs; only webhook/IMAP persistence or re-import in a folder with `specialUse = INBOX` is eligible.                                                                                                                                                  | AC-MAIL-041, AC-MAIL-045                                      |
...
| D-14 | Visible UI defaults to English with Russian and Latvian fully supported via a language switcher, all three under the finite NFR-I18N-001 allowlist (Q-1 superseded — see docs/design.md Changelog); light theme is primary; Q-3 makes the three design inputs normative for layouts, flows, and design, not legacy English visible copy.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Q-1; Q-2; Q-3                                                                                                      | NFR-I18N-001, NFR-THEME-001, NFR-DESIGN-001                                             |
...
| D-18 | Real providers enable incrementally in Q-11 order. Release accounting is 167 unconditional active criteria + 16 conditionally applicable AC-REAL criteria + 1 inactive criterion = 184 unique criteria. Each AC-REAL is PASS when its provider is enabled or NOT_ENABLED/N/A when disabled. GoDaddy still requires AC-REAL-GD-001..004 PASS or evidenced account/API ineligibility plus a dated explicit user exclusion; missing credentials alone never qualify.                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Q-11; `docs/remediation-plan.md` §5 task 3.4; Q-15                                                                 | FR-PROV-001, FR-REAL-001, FR-MAIL-008..009, FR-BCK-001..003, §12                        |
...
| D-23 | **Q-15 (2026-07-20), authorization amended 2026-07-24: Mail gains local workspace labels and automatic label rules.** Labels never synchronize to Gmail `X-GM-LABELS` or IMAP keywords. Any active-workspace member may manage definitions/rules and manually assign/remove labels or filter the list. A rule targets one account, one label, and INBOX-special-use incoming/imported mail only; supported predicates are exact sender and subject substring with AND semantics. Applied assignments survive rule edit/disable/delete. **Apply to existing mail** is optional, initially unchecked, uses the same INBOX eligibility, and is delivered through the durable Phase-5 backfill. Limits are 100 labels/workspace and 100 active rules/account. Regex/OR, nested labels, body matching, non-label actions, provider sync, and private labels are deferred. No unresolved question gates implementation. | Verbatim user request; approved `specs/mail-label-filtering-plan.md` v0.3; user authorization amendment 2026-07-24 | FR-MAIL-008..009, AC-MAIL-031..045, HC-16, Out of Scope                                 |
...
| R-9  | Rule evaluation could diverge between live and historical paths or across database collations.         | Use one canonical TypeScript matcher with NFKC/trim/lowercase semantics for webhook, IMAP, and backfill; do not substitute database `ILIKE`; verify AC-MAIL-040..041/045.                                                                                                                             |
| R-10 | Rule mutation can race with message persistence or overlapping rules can duplicate assignments.        | Serialize account-scoped rule mutation and eligible persistence under one transaction advisory lock, load rules after locking, define visibility by commit-before-snapshot order, rely on unique pairs/atomic writes, and run controlled create/edit/disable/delete lock-order tests for AC-MAIL-042. |
| R-11 | Interrupted historical application can skip, repeat, miscount, or silently stall work.                 | Immutable account/INBOX/rule/label snapshot, inclusive cutoff/exclusive cursor, exact processed/matched meanings, 200-row atomic batches, five-minute lease takeover, three-failure ceiling, defined manual retry, and live-overlap tests under AC-MAIL-045.                                          |
| R-12 | UIDVALIDITY reset recreates local messages and can detach manual labels.                               | Reapply automatic rules on import; document manual-label loss and reject unsafe nullable/non-unique `Message-ID` guessing until a reliable identity exists.                                                                                                                                           |
...
- **Mail account management and server power actions through an API token.** Creating, editing, deleting or testing an IMAP/SMTP account would carry a password through an assistant's context, and starting or stopping a VPS is not something an agent should reach; both stay operator actions in the dashboard (FR-AGT-002).
- **Cascading deletes through an API token.** A kanban board or column, a bookmark category, a Messages category or channel, and a mail account each take content the caller never saw; deleting them stays an operator decision (FR-AGT-002, generalizing AC-MSG-018).
- **POP3 mailbox access.** Only IMAP is supported for inbound synchronization.
- **Provider-side mail labels.** Gmail `X-GM-LABELS`, IMAP keyword synchronization, and any remote mailbox mutation caused by an Inspoter label are excluded; labels are local workspace data.
...
- **Dark theme and a theme switcher.** The light theme is required by Q-2. (Additional UI languages and a language switcher are no longer out of scope — Q-1 is superseded; English is now the default locale with Russian and Latvian fully supported. See docs/design.md Changelog for language and theme scope changes.)
...
- **M-10 (Real-provider outcomes and accounting):** PRD v3.12 contains exactly 167 unconditional active criteria + 16 conditionally applicable AC-REAL criteria + 1 inactive criterion = 184 unique criteria. All 167 unconditional active criteria PASS. Each conditional AC-REAL is PASS when its provider is enabled or NOT_ENABLED/N/A when disabled. GoDaddy meets this metric only through AC-REAL-GD-001..004 PASS or evidenced account/API ineligibility plus a dated explicit user exclusion in `docs/progress.md`; missing credentials alone never qualify.
...
- **M-14 (Mail labels and filter rules):** AC-MAIL-031..045 pass through service/API/component/Playwright coverage, including normalization and 40/80/320/200-character boundaries; 100-label and 100-active-rule boundary/concurrent conflicts; owner/member role matrix; INBOX-only webhook/IMAP/UIDVALIDITY parity and excluded-folder/move negatives; controlled create/edit/disable/delete lock ordering; combined pagination; accessibility at 375px/1440px; and exact backfill cutoff/cursor/count/lease/retry/live-overlap recovery.
...
1. All 167 unconditional active PRD v3.12 acceptance criteria are recorded as PASS in the current test plan; the sole inactive criterion, AC-MSG-008, is recorded as INACTIVE under Q-8. Workspace facets may advance independently through R2.7, but AC-WS-008/010/011 and the Workspaces family close only after R2.8.
...
5. Mail-label delivery records AC-MAIL-031..045 as PASS only after the owner/member role matrix; normalization/delete/limit boundary and concurrent tests; INBOX eligibility and excluded-folder/move tests; controlled rule create/edit/disable/delete lock-order races; exact backfill cutoff/cursor/count/lease/three-failure/manual-retry/live-overlap recovery; workspace isolation; desktop/375px keyboard/accessibility review; migration replay; rollback rehearsal; and explicit user verification of each planned phase.
...
No v3.14 requirement is gated by an unresolved product question. Q-1…Q-13, Q-14, and Q-15 are confirmed user decisions.
...
- **Q-15 — RESOLVED, BINDING (2026-07-20; authorization amended 2026-07-24):** implement local workspace labels, member-managed label definitions/rules/manual assignments/filtering, one-account incoming rules with exact-sender and subject-substring AND semantics, one target label, persistent assignments across rule lifecycle, and optional initially-unchecked durable historical application. Provider label sync, regex/OR, nesting, body matching, non-label actions, and private labels remain deferred.
...
All IDs are stable and must not be renumbered, reused, or transferred. v3.21 has exactly **229 unconditional active criteria + 16 conditionally applicable AC-REAL criteria + 1 inactive criterion = 246 unique criteria** (v3.20's 237 plus AC-MAIL-046..054):
...
- Mail: AC-MAIL-001..054, with AC-MAIL-007..030 added by Q-14 (accounts 007..011, sync 012..017, actions 018..021, send 022..025, attachments 026..028, folders/switching 029..030), AC-MAIL-031..045 added by Q-15 (labels 031..038, automatic rules/backfill 039..045), and AC-MAIL-046..054 added by the mail AI amendment (summary 046, reply draft 047, rule suggestion and its confirmation 048..049, disabled layer 050, call limit 051, invalid answer 052, prompt truncation 053, active provider 054)
...
- Agent surface: AC-AGT-001..008 added by §3.14 (scoped access 001, isolation 002, surface parity 003, attribution 004, no cascading delete 005, no mail credentials 006, author-only comment deletion 007, documented surface 008)
- Logs: AC-LOG-001..005
...
- Backup: AC-BCK-001..008, added by the workspace backup/restore amendment (round-trip 001, replace 002, merge 003, sealed-container rejects 004, global collision skips 005, WEBHOOK-mailbox merge 006, owner-only 007, partial export 008)
...
| FR-AGT-001..003 / AC-AGT-001..008   | User request 2026-08-23 («необходимо чтобы эти слои покрывали полноценную работу с kanban, bookmarks, contacts, messages, mail, services»); extends FR-MSG-004 and FR-CNT-006 to the remaining sections; `docs/architecture.md` §6.6 | Active; additive. No existing contract changed: AC-MSG-018's no-destructive-operations rule is generalized rather than relaxed, mail account credentials and server power actions stay out of scope (§10)                                                                    |
...
| FR-WH-003                           | Discord compatibility request (2026-08-02); `specs/discord-webhook-compatibility.md`                                                                                                                                                 | Active; additive — FR-WH-001/002 contracts and the `INSPOT` outgoing format are unchanged                                                                                                                                                                                    |
...
| NFR-I18N-001                        | Q-1; finite allowlist in NFR-I18N-001                                                                                                                                                                                                | Active; English default with Russian and Latvian fully supported via a language switcher, all three under the finite allowlist, with code/monospace technical exemptions; legacy English `specs/ui.md` copy is non-normative (Q-1 superseded — see docs/design.md Changelog) |
...
Added Latvian (`lv`) as a third operator-selectable UI locale via the existing language switcher; all 25 message namespaces translated. Updates HC-5, D-14, NFR-I18N-001, the §10 Out of Scope language/switcher bullet, and the NFR-I18N-001 traceability row to reflect three supported languages; the finite allowlist in NFR-I18N-001 is unchanged in content and now applies to all three locales. Q-1's superseded history (v3.9) is preserved, not rewritten. No FR/AC/Q/NFR ID was renumbered or reused.

### v3.21 — 2026-08-25 (Mail AI features)

Adds AC-MAIL-046..054 to §3.4 for the three model-assisted mail features: a reading-pane summary, a reply draft, and a filter-rule suggestion. The model only ever proposes: an existing deterministic path executes what the operator confirms, and none of the three writes anything by itself. Also specifies the disabled-layer, call-limit, invalid-answer and prompt-truncation paths, and how the active LLM provider is chosen. No existing AC ID was renumbered, reused, or retired.

### v3.20 — 2026-08-23 (agent surface across every section)

- Added §3.14 **FR-AGT-001..003** and **AC-AGT-001..008**: the agent surface FR-MSG-004 opened for Messages and FR-CNT-006 for Contacts now covers Bookmarks, Kanban, Mail and monitored Services as well — MCP tools on `POST /api/mcp` (44 → 109) and the REST family `/api/v1/**` (16 → 71 documented paths) over the same service layer, under one read and one write scope per section. Adds the `services:write` scope, bringing the token's scope set to 15.
- Recorded the three withheld classes of action as requirements rather than as implementation detail: cascading deletes (generalizing AC-MSG-018 to every section), mail account credentials, and server power actions. Added to §10.
- Recorded a **known documentation gap**: Kanban and monitored Services still have no feature requirements of their own — they were built after v3.19 and were never written up. §3.14 describes only their agent surface.
...
- Amended FR-DSH-002/003 and AC-DSH-012: calendar alert-day rows deep-link to a visible UTC-date filter in Alerts, and every widget kind may grow vertically to the shared 20-row limit independent of adjacent tile heights.
- Amended AC-MAIL-030: a workspace may persist exactly one default mailbox; Mail selects it unless a valid deep link overrides it, and deletion promotes a deterministic replacement.
- No AC ID was added, renumbered, reused, or retired.
...
- At this historical release Dashboards became the product home and replaced `/bookmarks` as the landing path. Executive Management later superseded this landing decision with `/management`; `/dashboards` remains available.
...
- Reversed the owner-only label-definition, rule-management, and backfill-control rules from AC-MAIL-031/039/043/044/045: any authenticated member of the active workspace may manage them.
- Workspace isolation remains mandatory; non-members receive no access and foreign resource identifiers keep the non-disclosing 404 contract.
- No AC ID was added, renumbered, or retired.
...
- Added §3.13 with **FR-BCK-001** (encrypted per-section export sealed with a passphrase: AES-256-GCM + scrypt + gzip, `.inspot-backup` format v1, manifest + per-model record arrays), **FR-BCK-002** (owner-only import in replace/merge modes with full id regeneration, single-transaction semantics, transient-state resets, and global collision skips for webhook tokens and provider bindings), and **FR-BCK-003** (secret portability: at-rest secrets travel decrypted inside the sealed archive and are re-encrypted with the local `CREDENTIAL_ENCRYPTION_KEY` at import) with **AC-BCK-001..008**. Accounting updated to 152 unconditional active + 16 conditional AC-REAL + 1 inactive = 169 unique criteria (D-18, §11 M-10, §12, Appendix B).
- No existing FR or AC ID was renumbered, reused, or retired.

### v3.12 — 2026-07-21 (Mail-label implementation reconciliation)

- Records the delivered local labels, manual assignments, label browsing,
  future webhook/IMAP rules, and durable optional historical application.
- Keeps AC-MAIL-031..045 unchanged and records guarded PASS evidence for the
  Phase 5 migration, performance, backup/restore, rollback, restart, regression,
  browser, and independent-review gates. User verification remains.
- Preserves the UIDVALIDITY limitation: automatic labels reapply on eligible
  re-import, while manual assignments may be lost; nullable/non-unique
  `Message-ID` values are never used to guess identity.

### v3.11 — 2026-07-20 (Mail labels and automatic label rules, Q-15)

- Recorded binding Q-15/D-23 from the verbatim request and reviewed `specs/mail-label-filtering-plan.md` v0.3: local workspace-scoped labels, owner-managed definitions/rules, member manual assignment/filtering, one-account incoming rules, exact-sender plus subject-substring AND matching, one target label, lifecycle-stable assignments, and optional initially-unchecked durable historical application.
- Added FR-MAIL-008/009 and exactly AC-MAIL-031..045 without renumbering AC-MAIL-001..030. Updated the Mail user story, affected NFR verification, hard constraints, assumptions/dependencies, negative scenarios, decisions, residual risks, out-of-scope boundaries, success metrics, Definition of Done, question disposition, AC index, and source traceability.
- Accounting is now 159 unconditional active + 16 conditionally applicable AC-REAL + 1 inactive = 176 unique criteria. Runtime PASS is not claimed; implementation and the new test evidence remain pending later phases.
- **v3.11 debate cycle 1 — CH-PRD-001: accepted_and_fixed.** §3.10 now makes owner-only label-definition/rule management and member manual assignment/removal/filtering part of the complete role matrix; AC-MAIL-031/032/039 and N-27 align on 403/no-write behavior.
- **v3.11 debate cycle 1 — CH-PRD-002: accepted_and_fixed.** AC-MAIL-041/045 and the eligibility contract define webhook/IMAP live and historical eligibility as persisted `specialUse = INBOX` only, exclude Archive/Junk/Trash/Drafts/Sent and moves, and treat UIDVALIDITY INBOX re-import as eligible.
- **v3.11 debate cycle 1 — CH-PRD-003: accepted_and_fixed.** AC-MAIL-042 defines the post-account-lock active-rule snapshot and commit-before/commit-after linearization boundary; M-14/DoD require controlled create/edit/disable/delete lock-order tests.
- **v3.11 debate cycle 1 — CH-PRD-004: accepted_and_fixed.** AC-MAIL-045 and the historical-run contract now fix account/INBOX/snapshot selection, inclusive cutoff/exclusive cursor traversal, count meanings, pre-labeled idempotency, atomic batches, five-minute lease, three-failure ceiling, manual retry, and live overlap.
- **v3.11 debate cycle 1 — CH-PRD-005: accepted_and_fixed.** AC-MAIL-031/039 and the transactional-limit contract now fix 100 labels/workspace, 100 active rules/account, 320/200 predicate limits, deterministic 409 outcomes, and boundary/concurrent verification.
- **v3.11 debate cycle 1 — CH-PRD-006: accepted_and_fixed.** FR/AC-MAIL-008/031 now define NFKC → trim → internal-whitespace collapse → lowercase normalization and 409 `LABEL_IN_USE` for any active/inactive rule reference; only unreferenced label deletion cascades assignments.

### v3.9 — 2026-07-20 (English default locale, Q-1 reversal)

- Recorded that Q-1's Russian-only disposition is superseded: English is now the default UI locale, with Russian fully supported and operator-selectable via a top-bar language switcher (implemented via `next-intl` locale routing; see docs/design.md Changelog for the activation decision). The finite NFR-I18N-001 allowlist now applies across both supported languages rather than solely as an exception to Russian.
- Updated HC-5, D-14, NFR-I18N-001, the §10 Out of Scope language/switcher bullet, and the NFR-I18N-001 traceability row to reflect the new scope. Q-1's history is preserved, not deleted; no FR/AC/Q/NFR ID was renumbered or reused.
