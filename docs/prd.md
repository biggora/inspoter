...
**Version:** v3.21
**Status:** Mail AI features specified and implemented
...
**NFR-I18N-001 (English default, Russian fully supported):** The finite allowlist of operator-visible non-translated terms, unchanged regardless of the active English or Russian locale, is: `inspoter`, `Cloudflare`, `Hetzner Cloud`, `Hetzner DNS`, `GoDaddy`, `DNS`, `VPS`, `IP`, `URL`, `TTL`, `HTTP`, `HTTPS`, `API`, and `JSON`. Technical identifiers, environment-variable names, and API snippets are exempt only when rendered as code or monospace. All unlisted operator-visible navigation, headings, labels, actions, validation messages, errors, loading states, empty states, and notifications must be fully localized in the active locale (English by default, Russian when selected via the top-bar language switcher). Legacy English visible copy in `specs/ui.md` is non-normative placeholder text and must be replaced with proper localized strings for both supported languages. _Verification:_ an automated visible-string scan against this allowlist plus a browser walkthrough of every route, dialog, loading state, error, and empty state finds no unlocalized text in either supported locale (Q-1, superseded — see docs/design.md Changelog).
...
| HC-5  | All visible UI defaults to English and is fully supported in Russian via an operator-selectable language switcher, except the finite allowlist in NFR-I18N-001 (which applies to both languages) and code/monospace technical exemptions.                                                                                                                                          | Q-1                                                                                             |
...
| D-14 | Visible UI defaults to English with Russian fully supported via a language switcher, both under the finite NFR-I18N-001 allowlist (Q-1 superseded — see docs/design.md Changelog); light theme is primary; Q-3 makes the three design inputs normative for layouts, flows, and design, not legacy English visible copy.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Q-1; Q-2; Q-3                                                                                                      | NFR-I18N-001, NFR-THEME-001, NFR-DESIGN-001                                             |
...
- **Dark theme and a theme switcher.** The light theme is required by Q-2. (Additional UI languages and a language switcher are no longer out of scope — Q-1 is superseded; English is now the default locale with Russian fully supported. See docs/design.md Changelog for language and theme scope changes.)
...
| NFR-I18N-001                        | Q-1; finite allowlist in NFR-I18N-001                                                                                                                                                                                                | Active; English default with Russian fully supported via a language switcher, both under the finite allowlist, with code/monospace technical exemptions; legacy English `specs/ui.md` copy is non-normative (Q-1 superseded — see docs/design.md Changelog) |
