...
**Version:** 1.24
**Status:** Browser-native WebMCP layer widened to the whole dashboard (98 tools, twelve domains, all page-independent); unit-verified, pending in-browser confirmation
**Owner:** Architect
**Date:** 2026-08-24
...
| CURRENT | Agent API — contacts (§6.6)            | `GET,POST /api/v1/contacts`; `GET,PATCH,DELETE /api/v1/contacts/[contactId]`; `GET,POST,DELETE /api/v1/contacts/[contactId]/photo`; `PATCH /api/v1/contacts/bulk`; `GET /api/v1/contacts/duplicates`; `POST /api/v1/contacts/merge`; `GET /api/v1/contacts/export`; `POST /api/v1/contacts/import`; `GET /api/v1/contacts/suggest`; `GET,POST /api/v1/contacts/labels`; `PATCH,DELETE /api/v1/contacts/labels/[labelId]`                                                                                                                                                                                                                                                                                                             | 11 / 18          |
...
Scopes are one read plus one write per domain (`src/lib/mcp/scopes.ts`, 15 in total). Writes through the REST family are journalled in `Activity` under the token's id and name; MCP tool calls are not, matching every pre-existing tool.
...
- **Anything taking a `File`/`Blob`** — contact import/export, contact photo upload, mail attachment download — since an agent cannot supply one.
