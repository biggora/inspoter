...
**CURRENT (2026-08-23):** `src/app/api` contains 194 `route.ts` files and 292 exported handlers: 92 GET, 88 POST, 57 PATCH, 49 DELETE, and 6 PUT. Beyond the v1.3 families below, later slices added Services (`/api/services/**`), provider credentials (`/api/credentials/**`), Authentik OIDC (`/api/auth/authentik/**`), the MCP endpoint (`/api/mcp`), Dashboards (`/api/dashboards/**`), and the token-authenticated agent API (`/api/v1/**`, §6.6); the Q-14 mail client expanded the Mail family as shown.
...
**CURRENT:** the same capability is published twice for a caller that holds an API token rather than a session: as MCP tools on `POST /api/mcp` (`src/lib/mcp/tools/*.ts`, 109 tools) and as the REST family `/api/v1/**` (71 paths in `specs/openapi.json`). Six sections are covered end to end — bookmarks, contacts, kanban, mail, messages and services — plus read access to alerts, servers and logs.
