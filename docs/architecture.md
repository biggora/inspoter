...
**Tool definition.** `src/lib/web-mcp/define-tool.ts` (`defineWebMcpTool()`) turns a zod schema and handler into a tool object, converting the schema to JSON Schema with `z.toJSONSchema()`. Because the advertised schema is advisory only — nothing in the browser enforces it — the wrapper re-validates input with `safeParse` before the handler runs. `execute` never throws or rejects: it resolves to the MCP result shape `{ content: [{ type: "text", text }], isError? }`, so a handler returns plain data on success and signals failure by throwing, which the wrapper reports in-band as `isError: true` for the agent to read and self-correct against.
