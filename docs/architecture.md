...
**Two API surfaces.** The W3C draft exposes the entry point as `document.modelContext` and marks `navigator.modelContext` deprecated, but the Chrome builds that implement the API today ship only the latter, and their `registerTool()` returns a registration handle carrying `unregister()` rather than the draft's Promise-plus-`AbortSignal`. Registering against one surface alone therefore fails silently in the field. `src/lib/web-mcp/support.ts` exports `getModelContexts()`, which collects every surface the browser exposes — `document` first, then `navigator`, deduplicated by object identity so a browser aliasing the two does not draw an `InvalidStateError` for a duplicate name — with `isWebMcpSupported()` as the boolean over it. `src/hooks/use-web-mcp-tool.ts` (`useWebMcpTool(tool, enabled)`) registers on each context and cleans up through both idioms: it aborts its `AbortController` and calls any returned `unregister()`. The effect is keyed on the tool's name so a new render's `execute` closure never forces a re-registration, and a synchronous throw or a rejected registration on one surface never prevents the others. `src/types/web-mcp.d.ts` declares both globals, the registration-handle union, and the `getTools`/`executeTool` members an agent uses to discover and invoke what the page registered.

**Tool definition.** `src/lib/web-mcp/define-tool.ts` (`defineWebMcpTool()`) turns a zod schema, a required human-readable `title` and a handler into a tool object, converting the schema to JSON Schema with `z.toJSONSchema()` under `io: "input"` — the schema describes what a caller sends, so a field carrying `.default()` is advertised as optional with its default as a hint, where the "output" default would list it as required and oblige a strict client to send a value the schema itself supplies. Because the advertised schema is advisory only — nothing in the browser enforces it — the wrapper re-validates input with `safeParse` before the handler runs. `execute` never throws or rejects: it resolves to the MCP result shape `{ content: [{ type: "text", text }], isError? }`, so a handler returns plain data on success and signals failure by throwing, which the wrapper reports in-band as `isError: true` for the agent to read and self-correct against.

Six tools are registered — two available on every dashboard route, four scoped to the page whose live state they operate on:

| Tool                  | Scope | Host component                                             | `api.ts` call                     | Annotations                                             |
| ---------------------- | ------ | ------------------------------------------------------------ | ------------------------------------ | ----------------------------------------------------------- |
| `note_search`          | global | `src/components/shell/web-mcp-global-tools.tsx`                | `notesApi.search`                  | `readOnlyHint: true`, `untrustedContentHint: true`         |
| `note_create`          | global | `src/components/shell/web-mcp-global-tools.tsx`                | `notesApi.create` (+ `notesApi.tree` to resolve a folder) | `readOnlyHint: false`, `untrustedContentHint: false`       |
| `kanban_move_card`     | board page | `src/components/kanban/kanban-board-view.tsx`                 | `cardsApi.move`                    | `readOnlyHint: false`, `untrustedContentHint: false`       |
| `kanban_create_card`   | board page | `src/components/kanban/kanban-board-view.tsx`                 | `cardsApi.create`                  | `readOnlyHint: false`, `untrustedContentHint: false`       |
| `alert_set_category`   | `/alerts` | `src/components/alerts/alerts-view.tsx`                       | `alertsApi.setCategoryBulk`        | `readOnlyHint: false`, `untrustedContentHint: false`       |
| `server_power_action`  | server detail page | `src/components/servers/server-detail-view.tsx`                | server power-action trigger (`use-server-power-action.ts`, itself backed by the servers `api.ts`) | `readOnlyHint: false`, `untrustedContentHint: false`       |

Each factory lives beside its feature's `api.ts` in a `web-mcp-tools.ts` (`src/components/{notes,kanban,alerts,servers}/web-mcp-tools.ts`), following the existing one-file-per-feature convention, and takes its collaborators as an injected context object so it stays unit-testable without React.

**Global versus page-scoped.** The notes tools depend on nothing but the authenticated session, so they mount from `src/components/shell/web-mcp-global-tools.tsx` — a client component rendering `null`, placed in `(dashboard)/layout.tsx` inside `RouteProgressProvider` but *outside* the `{children}` slot, which is the only part of that layout that swaps on navigation. They therefore stay registered across client-side route changes. The other four read live page state — the board's optimistic column order, the alerts table's reload token, the server whose detail page is open — so they are registered by the component that owns that state and disappear with it. `note_search` returns a compact projection (id, title, trimmed excerpt, folder) rather than note bodies, keeping tool output inside the recommended budget.

**Identifier resolution.** Kanban addresses a card or column by name-or-id: the handler resolves it as an exact id match, then an exact case-insensitive title match, then a unique case-insensitive title substring, reporting a candidate list on ambiguity. `note_create` resolves an optional folder the same way, against its own local copy of that helper. Alerts uses real ids — `alerts-view.tsx` gains a purely additive `data-alert-id={alert.id}` attribute on each `<TableRow>` so a page-reading agent can correlate a visible row with its id. Servers takes no identifier at all: `server_power_action` is registered only while that server's own detail page is mounted, with `enabled` gated on `powerActionsAvailable === true`, so "this server" comes from page context exactly like the on-page buttons.

**Out of scope (not implemented):** the declarative `toolname`/`tooldescription` HTML form attributes; `exposedTo` cross-origin tool sharing; the `toolchange` event, and `getTools()`/`executeTool()` as *callers* — the page registers tools and never consumes another document's, so those members are declared in the ambient types for the agent's use but never invoked here; `kanban_update_card` and label-setting; alert-category CRUD tools; `alertsApi.markAllRead()` as a tool; note updating and deletion; and WebMCP tools on the servers grid/list page (`servers-view.tsx`, as opposed to the single-server detail page).

**Verification.** The layer is covered by unit tests over both API surfaces (`tests/unit/web-mcp/`, `tests/unit/hooks/use-web-mcp-tool.test.tsx`) and per-feature factory tests (`tests/unit/{notes,kanban,alerts,servers}/web-mcp-tools.test.ts`), plus `e2e/web-mcp.spec.ts`, which fakes each surface through `page.addInitScript` rather than depending on a flag-gated browser build. Because no CI browser implements WebMCP natively, registration against a real agent remains a manual check.
