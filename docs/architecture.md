...
**CURRENT (2026-08-05):** `src/app/api` contains 108 `route.ts` files and 154 exported handlers: 46 GET, 53 POST, 26 PATCH, 26 DELETE, and 3 PUT. Beyond the v1.3 families below, later slices added Services (`/api/services/**`), provider credentials (`/api/credentials/**`), Authentik OIDC (`/api/auth/authentik/**`), the MCP endpoint (`/api/mcp`), Dashboards (`/api/dashboards/**`), and the token-authenticated Messages management family (`/api/v1/messages/**`, §6.6); the Q-14 mail client expanded the Mail family as shown.
...
| CURRENT | Messages management API (§6.6)         | `GET,POST /api/v1/messages/categories`; `PATCH /api/v1/messages/categories/[categoryId]`; `POST /api/v1/messages/channels`; `PATCH /api/v1/messages/channels/[channelId]`; `GET,POST /api/v1/messages/channels/[channelId]/messages`; `GET,POST /api/v1/messages/channels/[channelId]/webhooks`; `DELETE /api/v1/messages/channels/[channelId]/webhooks/[webhookId]` | 7 / 10           |
