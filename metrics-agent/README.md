...
| Server shows `stale` in the dashboard                             | No snapshot received for more than 180 seconds                           | `docker compose logs` on the host; a container that is up and logging `4xx` is the common case — work the rows above            |
| A rebuilt agent still misbehaves after redeploying                | The host pulled the same published image; the fix is not in it yet       | The image is built only by `release-metrics-agent.yml` on a **published GitHub release** — publish one, then `docker compose pull && docker compose up -d` |
