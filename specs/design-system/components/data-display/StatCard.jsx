...
const TONE = {
  ...
  secondary: { bg: 'oklch(var(--secondary-100))', fg: 'oklch(var(--secondary-600))' },
 ...
 */
export function StatCard({ icon, label, value, sub, subtitle, tone = 'primary', onClick, children, className = '' }) {
  useCSS();
  const t = TONE[tone] || TONE.primary;
  const Comp = onClick ? 'button' : 'div';
  return (
    <Comp onClick={onClick} className={`insp-stat ${onClick ? 'insp-stat--btn' : ''} ${className}`}>
      <div className="insp-stat__top">
        <span style={{ width: 32, height: 32, borderRadius: 'var(--radius-lg)', background: t.bg, color: t.fg, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, flexShrink: 0 }}>
          <i className={icon}></i>
        </span>
        <span className="insp-stat__label">{label}</span>
      </div>
      <p className="insp-stat__value">{value}{sub != null && <span className="insp-stat__sub">/{sub}</span>}</p>
