...
export default function AppLayout({ children }: { children: ReactNode }) {
  ...
  const renderNavItem = (item: NavItem) => (
    <Link
      key={item.path}
      to={item.path}
      className={`group ${navLinkClass(item.path)}`}
    >
  ...
  return (
    <div className="min-h-screen flex bg-background-50">
      ...
      >
        <div
          className={`flex items-center justify-between px-4 h-14 border-b border-background-200 shrink-0 ${collapsed ? "px-2 justify-center" : ""}`}
        >
          {collapsed ? (
            <Link
              to="/"
              className="w-8 h-8 flex items-center justify-center rounded-md bg-primary-100 no-underline shrink-0"
              title="Inspot"
            >
            ...
            <Link
              to="/bookmarks"
              className="font-heading text-lg font-bold text-foreground-900 no-underline"
            >
              Inspot
            </Link>
          )}
        ...
        <div
          className={`py-3 border-t border-background-200 ${collapsed ? "px-2" : "px-3"}`}
        >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 h-14 bg-background-50 border-b border-background-200 flex items-center justify-between px-4 shrink-0">
          <div className="flex items-center gap-3">
            ...
            >
              <i
                className={`text-lg ${collapsed ? "ri-menu-unfold-line" : "ri-menu-fold-line"}`}
              ></i>
            </button>
          ...
          <div className="flex items-center gap-1">
            ...
            >
              <i
                className={`text-lg ${resolvedTheme === "dark" ? "ri-sun-line" : "ri-moon-line"}`}
              ></i>
            </button>
            ...
            <div className="relative">
              ...
              >
                ...
                <i
                  className={`ri-arrow-down-s-line w-4 h-4 flex items-center justify-center text-foreground-400 transition-transform ${userMenuOpen ? "rotate-180" : ""}`}
                ></i>
              </button>
