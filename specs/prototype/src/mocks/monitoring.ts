...
function generateCpuHistory(base: number, variance: number, hours: number): MetricPoint[] {
  const points: MetricPoint[] = [];
  const now = new Date();
  for (let i = hours; i >= 0; i--) {
    const t = new Date(now.getTime() - i * 3600000);
    const val = Math.max(1, Math.min(100, base + (Math.random() - 0.5) * variance * 2));
    points.push({ timestamp: t.toISOString(), value: Math.round(val * 10) / 10 });
  }
  return points;
}

function generateMemoryHistory(base: number, variance: number, hours: number): MetricPoint[] {
  const points: MetricPoint[] = [];
  const now = new Date();
  let trend = base;
  for (let i = hours; i >= 0; i--) {
    ...
    points.push({ timestamp: t.toISOString(), value: Math.round(trend * 10) / 10 });
  }
  return points;
}
...
function generateDiskHistory(base: number, hours: number): MetricPoint[] {
  ...
  for (let i = hours; i >= 0; i--) {
    ...
    points.push({ timestamp: t.toISOString(), value: Math.round(used * 10) / 10 });
  }
  return points;
}
