...
const providerConfig: Record<DomainProvider, { icon: string; color: string; label: string }> = {
  Cloudflare: {
    icon: 'ri-cloud-line',
...
const statusConfig: Record<string, { label: string; className: string }> = {
  ...
  pending: { label: 'Ожидает', className: 'bg-secondary-100 text-secondary-800' },
  expired: { label: 'Истёк', className: 'bg-foreground-100 text-foreground-600' },
  transferred: { label: 'Перенесён', className: 'bg-secondary-100 text-secondary-700' },
};
    ...
    return (
      <div className="flex items-center justify-center min-h-[calc(100vh-3.5rem)] p-6">
        <div className="text-center max-w-sm animate-scale-in">
          ...
          <h3 className="font-heading text-lg font-semibold text-foreground-900 mb-2">Нет доменов</h3>
          <p className="text-sm text-foreground-500">
            В ваших аккаунтах пока нет доменов. Добавьте домен через панель провайдера, и он отобразится здесь.
          </p>
        </div>
      </div>
    );
...
function ProviderSection({
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-background-100">
                <th className="px-4 py-2.5 text-left text-xs font-medium text-foreground-500 whitespace-nowrap">Домен</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-foreground-500 whitespace-nowrap hidden sm:table-cell">Статус</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-foreground-500 whitespace-nowrap hidden md:table-cell">Истекает</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-foreground-500 whitespace-nowrap hidden lg:table-cell">NS</th>
            ...
            <tbody>
              {[1, 2, 3].map((i) => (
                <tr key={i} className="border-b border-background-50 last:border-0">
                  <td className="px-4 py-3"><div className="animate-skeleton h-4 w-36 rounded"></div></td>
                  <td className="px-4 py-3 hidden sm:table-cell"><div className="animate-skeleton h-5 w-20 rounded-full"></div></td>
                  <td className="px-4 py-3 hidden md:table-cell"><div className="animate-skeleton h-3 w-20 rounded"></div></td>
                  <td className="px-4 py-3 hidden lg:table-cell"><div className="animate-skeleton h-3 w-40 rounded"></div></td>
                  <td className="px-4 py-3 text-right"><div className="animate-skeleton h-7 w-16 rounded-lg ml-auto"></div></td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  if (state.error) {
    return (
      <div className="animate-fade-in">
        <div className="flex items-center gap-2 mb-2.5">
          <i className={`${cfg.icon} ${cfg.color} text-base w-5 h-5 flex items-center justify-center`}></i>
          <span className="font-heading font-semibold text-sm text-foreground-900">{cfg.label}</span>
        </div>
      ...
      <div className="rounded-xl border border-background-200 bg-background-50 overflow-hidden">
        ...
        <table className="w-full hidden sm:table">
          <thead>
            <tr className="border-b border-background-100">
              <th className="px-4 py-2.5 text-left text-xs font-medium text-foreground-500 whitespace-nowrap">Домен</th>
              <th className="px-4 py-2.5 text-left text-xs font-medium text-foreground-500 whitespace-nowrap">Статус</th>
              <th className="px-4 py-2.5 text-left text-xs font-medium text-foreground-500 whitespace-nowrap hidden md:table-cell">Истекает</th>
              <th className="px-4 py-2.5 text-left text-xs font-medium text-foreground-500 whitespace-nowrap hidden lg:table-cell">NS-серверы</th>
              <th className="px-4 py-2.5 text-right text-xs font-medium text-foreground-500 whitespace-nowrap w-28">DNS</th>
            </tr>
          </thead>
          <tbody>
            {state.domains.map((domain) => (
              ...
              >
                <td className="px-4 py-3">
                  <div>
                    <span className="text-sm font-medium text-foreground-900">{domain.name}</span>
                    <span className="text-xs text-foreground-400 ml-1.5">{domain.registrar}</span>
                  </div>
                </td>
                <td className="px-4 py-3">
                  <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${statusConfig[domain.status]?.className || ''}`}>
                ...
                <td className="px-4 py-3 text-sm text-foreground-600 hidden md:table-cell whitespace-nowrap">
                  <div className="flex items-center gap-1.5">
                    {domain.autoRenew && (
                      <i className="ri-loop-left-line text-xs text-accent-600 w-4 h-4 flex items-center justify-center" title="Автопродление"></i>
                    )}
                    {domain.expiresAt}
                  </div>
                </td>
                <td className="px-4 py-3 text-xs text-foreground-400 hidden lg:table-cell">
                  <div className="flex flex-wrap gap-1">
                    {domain.nameservers.map((ns) => (
                      <span key={ns} className="inline-block bg-background-100 rounded px-1.5 py-0.5 text-xs">
                        {ns}
                      </span>
                    ))}
                  </div>
        ...
        <div className="sm:hidden divide-y divide-background-50">
          {state.domains.map((domain) => (
            <div key={domain.id} className="px-4 py-3 space-y-2">
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs text-foreground-400">{domain.registrar}</span>
                <span className="text-xs text-foreground-500">{domain.expiresAt}{domain.autoRenew ? ' · автопродление' : ''}</span>
              </div>
              <div className="flex items-center justify-between pt-0.5">
                <span className="text-xs text-foreground-400 truncate max-w-[60%]">{domain.nameservers[0]}</span>
                <button
                  onClick={() => onViewDns(domain.id)}
                  className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-foreground-600 hover:text-foreground-900 hover:bg-background-200 transition-colors cursor-pointer whitespace-nowrap"
                >
