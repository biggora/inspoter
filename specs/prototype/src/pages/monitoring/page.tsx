...
export default function MonitoringPage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      avgCpu:
        Math.round(
          (servers.reduce((sum, s) => sum + s.cpu, 0) / servers.length) * 10,
        ) / 10,
      avgMemory:
        Math.round(
          (servers.reduce((sum, s) => sum + s.memory, 0) / servers.length) * 10,
        ) / 10,
      avgDisk:
        Math.round(
          (servers.reduce((sum, s) => sum + s.disk, 0) / servers.length) * 10,
        ) / 10,
      totalNetIn:
        Math.round(servers.reduce((sum, s) => sum + s.networkIn, 0) * 10) / 10,
      totalNetOut:
        Math.round(servers.reduce((sum, s) => sum + s.networkOut, 0) * 10) / 10,
    };
  }, [data]);
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="rounded-xl border border-background-200 bg-background-50 p-4">
          ...
          <p className="text-2xl font-heading font-bold text-foreground-950">
            {stats.avgCpu}
            <span className="text-base text-foreground-400 font-normal">%</span>
          </p>
          <div className="mt-1 h-1.5 w-full rounded-full bg-background-200 overflow-hidden">
            <div
              className="h-full rounded-full bg-primary-500 transition-all duration-500"
              style={{ width: `${Math.min(stats.avgCpu, 100)}%` }}
            ></div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-background-200 bg-background-50 p-4">
          ...
          <div className="mt-1 h-1.5 w-full rounded-full bg-background-200 overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min(stats.avgMemory, 100)}%` }}
            ></div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-background-200 bg-background-50 p-4">
          ...
          <p className="text-[11px] text-foreground-400 mt-1">
            ↓ {stats.totalNetIn} MB/s &middot; ↑ {stats.totalNetOut} MB/s
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
        ...
        <ChartContainer
          ...
          subtitle="Входящий / Исходящий, MB/s"
        >
          <ResponsiveContainer width="100%" height={260}>
        ...
        <div className="lg:col-span-2 rounded-xl border border-background-200 bg-background-50 p-5">
          <div className="mb-4">
            ...
            <p className="text-[11px] text-foreground-400 mt-0.5">
              Успешные и с ошибками, запросов/час
            </p>
          </div>
          <ResponsiveContainer width="100%" height={260}>
            <BarChart
            ...
            {pieData
              ...
              .map((entry, i) => (
                <div key={i} className="flex items-center gap-1.5">
                  ...
                  <span className="text-[11px] font-medium text-foreground-700">
                    {entry.value}
                  </span>
                </div>
              ))}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-background-100">
                <th className="text-left px-5 py-3 text-xs font-medium text-foreground-400 uppercase tracking-wide">
                  Сервер
                </th>
                <th className="text-left px-5 py-3 text-xs font-medium text-foreground-400 uppercase tracking-wide">
                  Статус
                </th>
                <th className="text-right px-5 py-3 text-xs font-medium text-foreground-400 uppercase tracking-wide">
                  CPU
                </th>
                <th className="text-right px-5 py-3 text-xs font-medium text-foreground-400 uppercase tracking-wide">
                  RAM
            ...
            <tbody>
              {data!.servers.map((server) => {
                ...
                return (
                  <tr
                    key={server.id}
                    className="border-b border-background-100 hover:bg-background-100/40 transition-colors"
                  >
                    <td className="px-5 py-3.5">
                      ...
                      <p className="text-[11px] text-foreground-400">
                        {server.host} &middot; {server.region}
                      </p>
                    </td>
                    <td className="px-5 py-3.5">
                      <span
                        className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px] font-medium ${statusInfo.cls} whitespace-nowrap`}
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${statusInfo.dot} ${server.status === "degraded" ? "animate-pulse" : ""}`}
                        ></span>
                        {statusInfo.label}
                      </span>
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      <span
                        className={`font-medium ${server.cpu > 80 ? "text-red-600" : server.cpu > 60 ? "text-amber-600" : "text-foreground-800"}`}
                      >
                        {server.cpu}%
                      </span>
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      <span
                        className={`font-medium ${server.memory > 85 ? "text-red-600" : server.memory > 70 ? "text-amber-600" : "text-foreground-800"}`}
                      >
                        {server.memory}%
                      </span>
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      <span
                        className={`font-medium ${server.disk > 85 ? "text-red-600" : server.disk > 70 ? "text-amber-600" : "text-foreground-800"}`}
                      >
                        {server.disk}%
                      </span>
                    </td>
                    <td className="px-5 py-3.5 text-right font-medium text-foreground-700">
                      {server.networkIn} MB/s
                    </td>
                    <td className="px-5 py-3.5 text-right font-medium text-foreground-700">
                      {server.networkOut} MB/s
                    </td>
                    <td className="px-5 py-3.5 text-right text-foreground-500">
                      {server.uptime}
                    </td>
                  </tr>
                );
