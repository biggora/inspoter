...
export default function BookmarksPage() {
  ...
  const submitCategoryRename = useCallback(() => {
    const cat = (dialog as { type: "category-rename"; category: Category })
      .category;
    if (!categoryName.trim()) {
      setCategoryNameError("Название категории обязательно.");
      return;
    ...
    setTimeout(() => {
      setCategories((prev) =>
        prev.map((c) =>
          c.id === cat.id ? { ...c, name: categoryName.trim() } : c,
        ),
      );
      setIsSubmitting(false);
      closeDialog();
  ...
  const submitCategoryDelete = useCallback(() => {
    const cat = (dialog as { type: "category-delete"; category: Category })
      .category;
    setIsSubmitting(true);
    setTimeout(() => {
  ...
  const submitBookmarkEdit = useCallback(() => {
    const bm = (dialog as { type: "bookmark-edit"; bookmark: Bookmark })
      .bookmark;
    if (!validateBookmark()) return;
    setIsSubmitting(true);
    setTimeout(() => {
  ...
  const submitBookmarkDelete = useCallback(() => {
    const bm = (dialog as { type: "bookmark-delete"; bookmark: Bookmark })
      .bookmark;
    setIsSubmitting(true);
    setTimeout(() => {
  ...
  return (
    <div className="p-6">
      ...
      <Modal
        open={
          dialog.type === "category-create" || dialog.type === "category-rename"
        }
        onClose={closeDialog}
        title={
      ...
      >
        {dialog.type === "category-delete" && (
          <div>
            ...
            {bookmarks.filter((b) => b.categoryId === dialog.category.id)
              .length > 0 && (
              <div className="mt-3 flex items-start gap-2 rounded-lg bg-primary-100/50 px-3 py-2.5">
                ...
                <p className="text-xs text-primary-700">
                  Все закладки в этой категории (
                  {
                    bookmarks.filter((b) => b.categoryId === dialog.category.id)
                      .length
                  }{" "}
                  шт.) будут также удалены. Это действие необратимо.
                </p>
              </div>
            )}
          </div>
      ...
      <Modal
        open={
          dialog.type === "bookmark-create" || dialog.type === "bookmark-edit"
        }
        onClose={closeDialog}
        title={
      ...
      >
        <div className="space-y-4">
          <div>
            ...
            />
            {bmNameError && (
              <p className="mt-1 text-xs text-primary-600">{bmNameError}</p>
            )}
          </div>
          ...
          <div>
            ...
            />
            {bmUrlError && (
              <p className="mt-1 text-xs text-primary-600">{bmUrlError}</p>
            )}
          </div>
