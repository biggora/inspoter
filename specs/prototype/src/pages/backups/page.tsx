...
const statusBadge: Record<string, { dot: string; cls: string; label: string }> =
  {
    active: {
      dot: "bg-accent-500",
      cls: "text-accent-700 bg-accent-100",
      label: "Активно",
    },
    paused: {
      dot: "bg-amber-500",
    ...
    in_progress: {
      dot: "bg-primary-500",
      cls: "text-primary-700 bg-primary-100",
      label: "В процессе",
    },
  };
...
export default function BackupsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const last24h = history.filter(
      (h) =>
        h.status === "completed" && new Date(h.completedAt!).getTime() > now24h,
    ).length;
    ...
    const successRate =
      totalAll > 0 ? Math.round((totalCompleted / totalAll) * 100) : 100;
  ...
  const hasActiveFilters =
    !!debouncedSearch || !!statusFilter || !!serverFilter || !!typeFilter;
      ...
      prev.map((s) => {
        ...
        return {
          ...
          nextRun:
            newStatus === "active"
              ? new Date(
                  Date.now() + 3600000 * (6 + Math.random() * 18),
                ).toISOString()
              : "—",
        };
      }),
      ...
      <div className="flex items-center justify-center min-h-[calc(100vh-3.5rem)] p-6">
        <div className="text-center max-w-sm animate-scale-in">
          ...
          <p className="text-sm text-foreground-500 mb-6">
            Проверьте подключение к хранилищу и попробуйте снова.
          </p>
          <button
            onClick={loadData}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="rounded-xl border border-background-200 bg-background-50 p-4">
          ...
          <p className="text-2xl font-heading font-bold text-foreground-950">
            ...
            <span className="text-base text-foreground-400 font-normal">
              /{schedules.length}
            </span>
          </p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-background-200 bg-background-100/50">
                  <th className="text-left py-3 px-4 text-[11px] font-medium text-foreground-400 uppercase tracking-wide whitespace-nowrap">
                    Сервер
                  </th>
                  <th className="text-left py-3 px-4 text-[11px] font-medium text-foreground-400 uppercase tracking-wide whitespace-nowrap">
                    Тип
                  ...
                  <th className="text-center py-3 px-4 text-[11px] font-medium text-foreground-400 uppercase tracking-wide whitespace-nowrap">
                    Статус
                  </th>
                    ...
                    >
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="text-xs font-medium text-foreground-900 whitespace-nowrap">
                            {sch.serverName}
                          </span>
                        </div>
                      </td>
                      <td className="py-3.5 px-4">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap ${backupTypeColors[sch.type]}`}
                        >
                          <i
                            className={`${backupTypeIcons[sch.type]} w-3 h-3 flex items-center justify-center`}
                          ></i>
                      ...
                      <td className="py-3.5 px-4">
                        {sch.nextRun === "—" ? (
                          ...
                          <div>
                            <p className="text-xs font-medium text-foreground-700 whitespace-nowrap">
                              {formatNextRun(sch.nextRun)}
                            </p>
                            <p className="text-[10px] text-foreground-400">
                              {formatAbsoluteTime(sch.nextRun)}
                            </p>
                          </div>
                        )}
                      </td>
                      <td className="py-3.5 px-4">
                        {sch.lastRun ? (
                          <p className="text-xs text-foreground-600 whitespace-nowrap">
                            {formatRelativeTime(sch.lastRun)}
                          </p>
                        ) : (
                          <span className="text-xs text-foreground-400">—</span>
                        )}
                      ...
                      <td className="py-3.5 px-4">
                        <span className="text-xs font-mono text-foreground-700 whitespace-nowrap">
                          {sch.lastSize || "—"}
                        </span>
                      </td>
                      <td className="py-3.5 px-4 text-center">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap ${sb.cls}`}
                        >
                          <span
                      ...
                      <td className="py-3.5 px-4 text-right">
                        ...
                        >
                          <i
                            className={`${sch.status === "active" ? "ri-pause-circle-line" : "ri-play-circle-line"} w-4 h-4 flex items-center justify-center`}
                          ></i>
          ...
          <div className="flex flex-wrap items-center gap-3 mb-5">
            ...
            <div className="relative" ref={serverRef}>
              ...
              >
                ...
                {serverFilter || "Сервер"}
                <i
                  className={`ri-arrow-down-s-line w-4 h-4 flex items-center justify-center transition-transform ${serverDropdownOpen ? "rotate-180" : ""}`}
                ></i>
              </button>
              {serverDropdownOpen && (
            ...
            <div className="relative" ref={typeRef}>
              ...
              >
                ...
                {typeFilter
                  ? backupTypeLabels[typeFilter as BackupType]
                  : "Тип"}
                <i
                  className={`ri-arrow-down-s-line w-4 h-4 flex items-center justify-center transition-transform ${typeDropdownOpen ? "rotate-180" : ""}`}
                ></i>
              </button>
              {typeDropdownOpen && (
                <div className="absolute top-full left-0 mt-1 z-30 w-44 bg-background-50 border border-background-200 rounded-lg shadow-lg animate-scale-in overflow-hidden">
                  <div className="py-1">
                    ...
                    {(["full", "incremental", "database"] as BackupType[]).map(
                      (t) => (
                        <button
                          key={t}
                          onClick={() => {
                        ...
                        >
                          <i
                            className={`${backupTypeIcons[t]} w-4 h-4 flex items-center justify-center`}
                          ></i>
                          {backupTypeLabels[t]}
                        </button>
                      ),
                    ...
                    {pagedHistory.map((entry) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-3 px-4">
                            <span
                              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap ${backupTypeColors[entry.type]}`}
                            >
                              <i
                                className={`${backupTypeIcons[entry.type]} w-3 h-3 flex items-center justify-center`}
                              ></i>
                          ...
                          <td className="py-3 px-4">
                            <div>
                              <span
                                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap ${eb.cls}`}
                              >
                                <span
                                  className={`w-1.5 h-1.5 rounded-full ${eb.dot} ${entry.status === "in_progress" ? "animate-pulse" : ""}`}
                                ></span>
                                {eb.label}
                              </span>
                              {entry.errorMessage && (
                          ...
                          <td className="py-3 px-4">
                            <div>
                              <p className="text-xs text-foreground-700 whitespace-nowrap">
