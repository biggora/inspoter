...
const severityColors: Record<
  string,
  { dot: string; text: string; bg: string; icon: string }
> = {
  critical: {
    dot: "bg-red-500",
    text: "text-red-700",
...
export default function DashboardPage() {
  ...
  const stats = useMemo(
    () => ({
      ...
      alertsCritical: mockAlerts.filter(
        (a) => a.severity === "critical" && !a.acknowledged,
      ).length,
      alertsWarning: mockAlerts.filter(
        (a) => a.severity === "warning" && !a.acknowledged,
      ).length,
      alertsInfo: mockAlerts.filter(
  ...
  const domainsWithHealth = useMemo(() => {
    return mockDomains
      .map((d) => {
        ...
        else if (d.status === "pending" || d.status === "transferred")
          health = "warning";
          ...
          <div className="flex items-center gap-1.5 mt-2">
            <div className="flex-1 h-1.5 rounded-full bg-background-200 overflow-hidden">
              <div
                ...
                style={{
                  width: `${stats.emailsTotal > 0 ? ((stats.emailsTotal - stats.emailsUnread) / stats.emailsTotal) * 100 : 0}%`,
                }}
              ></div>
            </div>
            <span className="text-[10px] text-foreground-400">
              {stats.emailsTotal > 0
                ? Math.round(
                    ((stats.emailsTotal - stats.emailsUnread) /
                      stats.emailsTotal) *
                      100,
                  )
                : 0}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-background-100">
                  <th className="text-left py-2.5 pr-3 text-[11px] font-medium text-foreground-400 uppercase tracking-wide whitespace-nowrap">
                    Сервер
                  </th>
                  <th className="text-left py-2.5 pr-3 text-[11px] font-medium text-foreground-400 uppercase tracking-wide whitespace-nowrap">
                    Статус
                  </th>
                  <th className="text-right py-2.5 pr-3 text-[11px] font-medium text-foreground-400 uppercase tracking-wide whitespace-nowrap">
                    CPU
                  </th>
                  <th className="text-right py-2.5 text-[11px] font-medium text-foreground-400 uppercase tracking-wide whitespace-nowrap">
                    RAM
                  </th>
                </tr>
              </thead>
              <tbody>
                {mockServers.map((server) => {
                  ...
                  return (
                    <tr
                      key={server.id}
                      className="border-b border-background-100 last:border-0 hover:bg-background-100/40 transition-colors"
                    >
                      <td className="py-3 pr-3">
                        <p className="text-xs font-medium text-foreground-900 whitespace-nowrap">
                          {server.name}
                        </p>
                      ...
                      <td className="py-3 pr-3">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap ${statusCls}`}
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${isRunning ? "bg-accent-500" : "bg-foreground-300"}`}
                          ></span>
                          {statusLabel}
                        </span>
                      </td>
            ...
            <div className="space-y-1.5">
              {recentCriticalAlerts.map((alert) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-7 h-7 rounded-md ${sev.bg} flex items-center justify-center shrink-0 mt-0.5`}
                    >
                      <i className={`${sev.icon} text-xs ${sev.text}`}></i>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium text-foreground-900 truncate">
                        {alert.title}
                      </p>
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {!alert.acknowledged && (
                          <span
                            className={`text-[10px] font-medium ${sev.text}`}
                          >
                            Не подтверждено
                          </span>
                        )}
      ...
      <div className="rounded-xl border border-background-200 bg-background-50 p-5 mb-5">
        ...
        {recentBackups.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-sm text-foreground-500">
              Нет завершённых бэкапов
            </p>
            <p className="text-[11px] text-foreground-400 mt-0.5">
              Расписания бэкапов можно настроить в разделе Бэкапы
            </p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${isCompleted ? "bg-accent-500" : "bg-red-500"}`}
                      ></span>
                  ...
                  {/* Type badge */}
                  <span
                    className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium w-fit whitespace-nowrap ${typeColor}`}
                  >
                  ...
                  {isFailed && backup.errorMessage && (
                    <div className="flex items-start gap-1 mt-0.5">
                      ...
                      <span className="text-[10px] text-red-500 line-clamp-2 leading-tight">
                        {backup.errorMessage}
                      </span>
                    </div>
                  )}
            ...
            {recentEmails.map((email) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <p
                      className={`text-xs truncate ${email.isRead ? "font-medium text-foreground-700" : "font-semibold text-foreground-900"}`}
                    >
                      {email.fromName}
                    </p>
                  ...
                  <p className="text-xs text-foreground-700 mt-0.5 line-clamp-1">
                    {email.subject}
                  </p>
                  {!email.isRead && (
          ...
          <div className="space-y-1">
            {domainsWithHealth.map((domain) => {
              const healthConfig = {
                ...
                critical: {
                  dot: "bg-red-500",
                  label: domain.status === "expired" ? "Истёк" : "Истёк",
                  cls: "text-red-700 bg-red-100",
                },
              }[domain.health];
              ...
              return (
                <div
                  ...
                  onClick={() =>
                    window.REACT_APP_NAVIGATE?.(`/domains/dns/${domain.id}`)
                  }
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-foreground-400">
                      {domain.provider} &middot; {domain.registrar}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap ${healthConfig.cls}`}
                    >
                      <span
