...
export function ProfileTab({
  ...
  const inputClass =
    "w-full px-3 py-2.5 rounded-lg border border-background-200 bg-background-50 text-sm text-foreground-900 placeholder:text-foreground-400 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-300 transition-colors";
    ...
    <div className="animate-fade-in">
      <div className="max-w-2xl">
        ...
        <div className="flex items-center gap-5 mb-8 p-5 rounded-xl border border-background-200 bg-background-50">
          ...
          <div>
            <h4 className="font-heading text-base font-semibold text-foreground-900">
              {form.name}
            </h4>
            <p className="text-sm text-foreground-500">{form.role}</p>
            <p className="text-xs text-foreground-400 mt-1">
              {form.company} &middot; {form.position}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              maxLength={500}
            ></textarea>
            <p className="text-[10px] text-foreground-400 mt-1 text-right">
              {form.bio.length}/500
            </p>
          </div>
        ...
        <div className="mt-5 flex items-center gap-4 text-xs text-foreground-400">
          <span>
            В команде с{" "}
            {new Date(form.joinedAt).toLocaleDateString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
            })}
          ...
          <span>
            Активен:{" "}
            {new Date(form.lastActive).toLocaleString("ru-RU", {
              day: "numeric",
              month: "short",
              hour: "2-digit",
              minute: "2-digit",
