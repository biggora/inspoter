...
interface ServerCardProps {
  server: Server;
  onPowerAction: (serverId: string, action: 'start' | 'stop' | 'restart') => void;
  error: string | null;
}

const statusConfig: Record<ServerStatus, { label: string; color: string; dotColor: string }> = {
  running: { label: 'Running', color: 'bg-accent-100 text-accent-700', dotColor: 'bg-accent-500' },
  stopped: { label: 'Stopped', color: 'bg-secondary-100 text-secondary-700', dotColor: 'bg-secondary-400' },
  starting: { label: 'Starting…', color: 'bg-primary-100 text-primary-700', dotColor: 'bg-primary-400' },
  stopping: { label: 'Stopping…', color: 'bg-primary-100 text-primary-700', dotColor: 'bg-primary-400' },
  restarting: { label: 'Restarting…', color: 'bg-primary-100 text-primary-700', dotColor: 'bg-primary-400' },
};
...
export default function ServerCard({ server, onPowerAction, error }: ServerCardProps) {
  const [menuOpen, setMenuOpen] = useState<'start' | 'stop' | 'restart' | null>(null);
  const config = statusConfig[server.status];
  const busy = isTransitional(server.status);

  const availableActions: { action: 'start' | 'stop' | 'restart'; label: string; icon: string; confirmTitle: string; confirmText: string }[] = [];
    ...
    <div className="rounded-xl border border-background-200 bg-background-50 overflow-hidden transition-colors hover:border-background-300">
      <div className="px-4 py-3.5 border-b border-background-100 flex items-center justify-between">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <h4 className="font-heading text-sm font-semibold text-foreground-900 truncate">{server.name}</h4>
        ...
        >
          <span className={`w-1.5 h-1.5 rounded-full ${busy ? 'animate-pulse' : ''} ${config.dotColor}`}></span>
          {config.label}
        </span>
      </div>
      ...
      <div className="px-4 py-3 space-y-1.5">
        ...
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="text-foreground-800 font-medium">{server.location}</span>
        </div>
      </div>
      ...
      <div className="px-4 py-3 border-t border-background-100 flex items-center gap-2">
        {availableActions.map((act) => (
          <div key={act.action} className="relative">
            <button
              onClick={() => setMenuOpen(menuOpen === act.action ? null : act.action)}
              disabled={busy}
            ...
            >
              <i className={`${act.icon} w-4 h-4 flex items-center justify-center`}></i>
              {act.label}
            </button>
            {menuOpen === act.action && (
              <>
                <div className="fixed inset-0 z-30" onClick={() => setMenuOpen(null)} aria-hidden="true" />
                <div className="absolute left-0 bottom-full mb-2 w-72 rounded-lg border border-background-200 bg-background-50 shadow-lg animate-scale-in z-40">
                  <div className="px-4 py-3">
                    <h5 className="font-heading text-sm font-semibold text-foreground-900 mb-1">{act.confirmTitle}</h5>
                    <p className="text-xs text-foreground-600 mb-3">{act.confirmText}</p>
        ...
        {availableActions.length === 0 && !busy && (
          <span className="text-xs text-foreground-400">Нет доступных действий</span>
        )}
        {busy && (
