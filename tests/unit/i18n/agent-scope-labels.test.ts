...
// to MCP_SCOPES without a label makes next-intl throw MISSING_MESSAGE while
// rendering — visible only in the server log. This is the guard that turns
// that into a failing test instead.

const DOMAINS = [...new Set(MCP_SCOPES.map((scope) => scope.split(":")[0]))];
...
describe("agent scope labels", () => {
  it.each(DOMAINS)(
    "has a label for the %s domain in both locales",
    (domain) => {
